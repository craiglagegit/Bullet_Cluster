...
void move_galaxies(struct galaxy_data *g1, struct galaxy_data *g2, double ip, double theta, double rstart, double velincrease)
{
  int i;
  double vstart;
  double M, Rstart;
  ...
  vstart = -sqrt(2 * G * M / Rstart);

  pos[0] = Rstart;
  pos[1] = -ip * cos(theta * PI / 180.0);
  pos[2] = ip * sin(theta * PI / 180.0);

  vel[0] = vstart * velincrease;
  vel[1] = 0;
  vel[2] = 0;

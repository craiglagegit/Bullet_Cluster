...
import pysubs_nas_6Jun13 as pysubs
...
cmd=sys.argv
Counter = int(cmd[1])
MagScaleFactor = float(cmd[2]) # Generated random fields with peak value of 1.0. This multiplier sets the actual value in uG.
FieldMode = cmd[3] # Generated random fields with peak value of 1.0. This multiplier sets the actual value in uG.
Tbg = float(cmd[4]) # Generated random fields with peak value of 1.0. This multiplier sets the actual value in uG.
LogRhoMin = float(cmd[5]) # Minimum density outside the initial clusters
...
for i in range(NumGrids): 
	...
	if enzparam.ComovingCoordinates==1:
		xmin.append(GadgetBoxLeftEdge[0] + enzparam.CosmologySimulationGridLeftEdge[i][0] * (GadgetBoxRightEdge[0]-GadgetBoxLeftEdge[0]))
		xmax.append(GadgetBoxLeftEdge[0] + enzparam.CosmologySimulationGridRightEdge[i][0] * (GadgetBoxRightEdge[0]-GadgetBoxLeftEdge[0]))
		ymin.append(GadgetBoxLeftEdge[1] + enzparam.CosmologySimulationGridLeftEdge[i][1] * (GadgetBoxRightEdge[1]-GadgetBoxLeftEdge[1]))
	...
	Rhomin = 10**LogRhoMin
	for ii in range(nx[i]):
		for jj in range(ny[i]):
			for kk in range(nz[i]):
				if Density[i].data[ii,jj,kk] < Rhomin:
					Density[i].data[ii,jj,kk] = Rhomin
					InternalEnergy[i].data[ii,jj,kk] = Umin
	SmoothedDensity.append(gaussian_filter(Density[i].data,3.0))
	LocalRhoMax = SmoothedDensity[i].max()
	if LocalRhoMax > RhoMax:
		RhoMax = LocalRhoMax

for i in range(NumGrids): 
	# Read in the magnetic field
	bx = pysubs.Array3d(xmin[i],xmax[i],nx[i],ymin[i],ymax[i],ny[i],zmin[i],zmax[i],nz[i])
	by = pysubs.Array3d(xmin[i],xmax[i],nx[i],ymin[i],ymax[i],ny[i],zmin[i],zmax[i],nz[i])
	bz = pysubs.Array3d(xmin[i],xmax[i],nx[i],ymin[i],ymax[i],ny[i],zmin[i],zmax[i],nz[i])
	phi = pysubs.Array3d(xmin[i],xmax[i],nx[i],ymin[i],ymax[i],ny[i],zmin[i],zmax[i],nz[i])

	bx1 = pysubs.Array3d(xmin[i],xmax[i],nx[i],ymin[i],ymax[i],ny[i],zmin[i],zmax[i],nz[i])
	by1 = pysubs.Array3d(xmin[i],xmax[i],nx[i],ymin[i],ymax[i],ny[i],zmin[i],zmax[i],nz[i])
	bz1 = pysubs.Array3d(xmin[i],xmax[i],nx[i],ymin[i],ymax[i],ny[i],zmin[i],zmax[i],nz[i])

	infile = open('../../../../mag_files/magField_'+str(Counter)+'_'+str(i+1)+'.dat','r')
	lines = infile.readlines()
	infile.close()
	for nn in range(len(lines)):
		kk = (nn % (nz[i]*ny[i])) % nz[i]
		jj = (nn % (nz[i]*ny[i]) - kk) / nz[i]
		ii = (nn - jj*nz[i] - kk) / (nz[i] * ny[i])
		bx1.data[ii,jj,kk] = float(lines[nn].split()[0]) * MagScaleFactor * pow(SmoothedDensity[i][ii,jj,kk] / RhoMax, 2.0/3.0) * 1.0E-6 / EnzoBField
		by1.data[ii,jj,kk] = float(lines[nn].split()[1]) * MagScaleFactor * pow(SmoothedDensity[i][ii,jj,kk] / RhoMax, 2.0/3.0) * 1.0E-6 / EnzoBField
		bz1.data[ii,jj,kk] = float(lines[nn].split()[2]) * MagScaleFactor * pow(SmoothedDensity[i][ii,jj,kk] / RhoMax, 2.0/3.0) * 1.0E-6 / EnzoBField
	...
	for ii in range(nx[i]):
		for jj in range(ny[i]):
			for kk in range(nz[i]):
				TotalEnergy[i].data[ii,jj,kk] = InternalEnergy[i].data[ii,jj,kk] + (Vx[i].data[ii,jj,kk]**2.0 + Vy[i].data[ii,jj,kk]**2.0 + Vz[i].data[ii,jj,kk]**2.0)/2.0 + (bx.data[ii,jj,kk]**2.0 + by.data[ii,jj,kk]**2.0 + bz.data[ii,jj,kk]**2.0) / (8.0 * pi * Density[i].data[ii,jj,kk]) 


	print 'Max internal energy = %.2g, Min Internal Energy = %.2g, Umin = %.2g \n'%(InternalEnergy[i].data.max(),InternalEnergy[i].data.min(),Umin)
	print 'Max total energy = %.2g, Min total Energy = %.2g \n'%(TotalEnergy[i].data.max(),TotalEnergy[i].data.min())


	mass[i].data                = MassConversion * mass[i].data.reshape((1,)+mass[i].data.shape)
	Density[i].data             = DensityConversion * Density[i].data.reshape((1,)+Density[i].data.shape) 
	TotalEnergy[i].data         = (VelocityConversion**2) * TotalEnergy[i].data.reshape((1,)+TotalEnergy[i].data.shape)
	InternalEnergy[i].data      = (VelocityConversion**2) * InternalEnergy[i].data.reshape((1,)+InternalEnergy[i].data.shape)
	Vx[i].data                  = VelocityConversion * Vx[i].data.reshape((1,)+Vx[i].data.shape)
	Vy[i].data                  = VelocityConversion * Vy[i].data.reshape((1,)+Vy[i].data.shape)
	Vz[i].data                  = VelocityConversion * Vz[i].data.reshape((1,)+Vz[i].data.shape)
	...
	if NumGrids==1:
		pysubs.WriteEnzoInitFile(Density[i].data, enzparam.CosmologySimulationDensityName,enzparam.CosmologySimulationDensityName)
		pysubs.WriteEnzoInitFile(Vx[i].data, enzparam.CosmologySimulationVelocity1Name, enzparam.CosmologySimulationVelocity1Name)
		pysubs.WriteEnzoInitFile(Vy[i].data, enzparam.CosmologySimulationVelocity2Name, enzparam.CosmologySimulationVelocity2Name)
		pysubs.WriteEnzoInitFile(Vz[i].data, enzparam.CosmologySimulationVelocity3Name, enzparam.CosmologySimulationVelocity3Name)
		pysubs.WriteEnzoInitFile(InternalEnergy[i].data, enzparam.CosmologySimulationGasEnergyName, enzparam.CosmologySimulationGasEnergyName)
		pysubs.WriteEnzoInitFile(TotalEnergy[i].data, enzparam.CosmologySimulationTotalEnergyName, enzparam.CosmologySimulationTotalEnergyName)
		...
		pysubs.WriteEnzoInitFile(Density[i].data, enzparam.CosmologySimulationDensityName+"."+str(i),enzparam.CosmologySimulationDensityName+"."+str(i))
		pysubs.WriteEnzoInitFile(Vx[i].data, enzparam.CosmologySimulationVelocity1Name+"."+str(i), enzparam.CosmologySimulationVelocity1Name+"."+str(i))
		pysubs.WriteEnzoInitFile(Vy[i].data, enzparam.CosmologySimulationVelocity2Name+"."+str(i), enzparam.CosmologySimulationVelocity2Name+"."+str(i))
		pysubs.WriteEnzoInitFile(Vz[i].data, enzparam.CosmologySimulationVelocity3Name+"."+str(i), enzparam.CosmologySimulationVelocity3Name+"."+str(i))
		pysubs.WriteEnzoInitFile(InternalEnergy[i].data, enzparam.CosmologySimulationGasEnergyName+"."+str(i), enzparam.CosmologySimulationGasEnergyName+"."+str(i))
		pysubs.WriteEnzoInitFile(TotalEnergy[i].data, enzparam.CosmologySimulationTotalEnergyName+"."+str(i), enzparam.CosmologySimulationTotalEnergyName+"."+str(i))

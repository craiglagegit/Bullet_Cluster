		...
		for (sn,Ps,Th) in [(snap-1,Psi,Theta),(snap+1,Psi,Theta),(snap,Psi+DeltaPsi,Theta),(snap,Psi-DeltaPsi,Theta),\
			...
			if FomMatrix[sn,Ps,Th][0] > 999.0: # This means it's a boundary
				...
				Theta = Th
                                ...
                                Vz = abs(dot(R,RelVel)[2]) # This is the observed Z_Velocity of the bullet relative to the CM
                                if Vz > MaxVz or Vz < MinVz: # If Vz is outside of observed Mean +/- 4 Sigma
                                    print 'In final windowing, Vz Outside allowed Vz, snap = %d, theta = %.3f, psi = %.3f, Vz = %.2f, MinVz = %.2f, MaxVz = %.2f\n'%(snap,theta, psi, Vz,MinVz,MaxVz)
                                    sys.stdout.flush()
...
def ComparisonPlot(data,sim,title,filled_levels=None,line_levels=None,cmap=None,line_plot_multiplier=1.0,line_plot_yticks=[0.0,1.0],simtime=0.80,fom=4.0,line=[80,53,0.26],legend_location=[0.50,1.0],take_log=False):
    ...
    ax4.plot(simslice.x,simslice.data*line_plot_multiplier,label="Sim",linewidth=3.0,ls='-',color='g',marker='o',mec='g',markevery=5)
...
def MultiLinePlot(data,sim,title,filled_levels=None,line_levels=None,cmap=None,line_plot_yticks=[0.0,1.0],simtime=0.80,fom=4.0,line1=[78,56,-0.24],line2=[78,56,0.24],line3=[78,56,1.02],legend_location1=[0.50,1.0],legend_location2=[0.50,1.0],legend_location3=[0.50,1.0],take_log=False):
    ...
    ax2.plot(simslice1.x,log10(simslice1.data),label="Sim",linewidth=3.0,ls='-',color='g',marker='o',mec='g',markevery=5)
    ...
    ax3.plot(simslice2.x,log10(simslice2.data),label="Sim",linewidth=3.0,ls='-',color='g',marker='o',mec='g',markevery=5)
    ax3.set_xticks([-500.0,0.0,500.0])
    ax3.set_xlabel('kpc')
    ...
    ax4.plot(simslice3.x,log10(simslice3.data),label="Sim",linewidth=3.0,ls='-',color='g',marker='o',mec='g',markevery=5)

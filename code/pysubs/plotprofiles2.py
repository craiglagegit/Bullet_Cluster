...
filenamebase = 'plots/'
pp=PdfPages('plots/Graph_Profiles_Early.pdf')
...
for snap in [2,16,30]:#[10,51,99]:
	...
	[TrueTemp,Temp,BMag] = pysubs.ProjectEnzoTemp(pf,mass,phi=0.0,theta=0.0,psi=0.0,zmin=zmin,zmax=zmax)	
	print "X-axis, TrueTemp = ",TrueTemp.data, "\nTemp = ",Temp.data
	subplot(4,3,1)
	title('X-axis Log DM Density',fontsize=10)	
	plot(mass.x-Centroids[1,1,0], (log10(DM.data[:,0])+log10(DM.data[:,1])+log10(DM.data[:,2]))/3.0, label = 'Time = %.2f Gy'%simtime)
	...
	plot(mass.x-Centroids[1,1,0], (log10(mass.data[:,0])+log10(mass.data[:,1])+log10(mass.data[:,2]))/3.0, label = 'Time = %.2f Gy'%simtime)
	xlim(-plotscale,plotscale)
	ylim(-2.0,0.0)
	gca().set_xticks([-plotscale,0.0,plotscale])
	subplot(4,3,7)
	title('X-axis Gas Temp(keV)',fontsize=10)	
	plot(mass.x-Centroids[1,1,0], (TrueTemp.data[:,0]+TrueTemp.data[:,1]+TrueTemp.data[:,2])/3.0 , label = 'Time = %.2f Gy'%simtime)
	xlim(-plotscale,plotscale)
	ylim(0.0,40.0)
	gca().set_xticks([-plotscale,0.0,plotscale])
	...
	[TrueTemp,Temp,BMag] = pysubs.ProjectEnzoTemp(pf,mass,phi=pi/2.0,theta=0.0,psi=0.0,zmin=zmin,zmax=zmax)	
	...
	[TrueTemp,Temp,BMag] = pysubs.ProjectEnzoTemp(pf,mass,phi=pi/2.0,theta=pi/2.0,psi=0.0,zmin=zmin,zmax=zmax)	

...
def ProjectEnzoRMVz(pf,mass,phi=0.0,theta=0.0,psi=0.0,zmin=-3000.0,zmax=3000.0):
    # Projects 3D Enzo data onto a 2D grid
    # Returns Rotation Measure and Projected Vz

    start = time.time()
    xpixels=mass.nx
    ypixels=mass.ny
    PixelArea = mass.dx * mass.dy
    Vpar=Array2d(mass.xmin,mass.xmax,mass.nx,mass.ymin,mass.ymax,mass.ny)
    RM=Array2d(mass.xmin,mass.xmax,mass.nx,mass.ymin,mass.ymax,mass.ny)
    ...
    for grid in pf.h.grids:
        grid.set_field_parameter('Normal_Vector',normal_vector)

    add_field('VParFlux', function = _EnzoVparFlux, units=r"\rm{cm}^{-2}\rm{s}^{-2}", projected_units=r"\rm{cm}^{-1}\rm{s}^{-2}", validators=[ValidateParameter('ApecData'), ValidateParameter('Normal_Vector')], take_log=False)
    add_field('XRay1', function = _EnzoXRay1, units=r"\rm{cm}^{-3}\rm{s}^{-1}", projected_units=r"\rm{cm}^{-2}\rm{s}^{-1}", validators=[ValidateParameter('ApecData')], take_log=False)
    add_field('BparDensity', function = _EnzoBparDensity,validators=[ValidateParameter('Normal_Vector')],take_log=False)

    VFactor    = - 1.0 / 1.0E5 # Put into km/sec - is away from us 
    RMFactor = BulletConstants.NeNi * pow(BulletConstants.esu,3.0) * 1.0E4 / (2 * pi * BulletConstants.mstar * pow(BulletConstants.me * BulletConstants.erg_per_eV, 2.0)) 
    projcam=ProjectionCamera(center,normal_vector,width,resolution,"VParFlux",north_vector=north_vector,pf=pf, interpolated=True)
    Vpar.data = projcam.snapshot()[:,:,0]
    projcam=ProjectionCamera(center,normal_vector,width,resolution,"XRay1",north_vector=north_vector,pf=pf, interpolated=True)
    Vpar.data = Vpar.data / projcam.snapshot()[:,:,0] * VFactor
    projcam=ProjectionCamera(center,normal_vector,width,resolution,"BparDensity",north_vector=north_vector,pf=pf, interpolated=True)
    RM.data = projcam.snapshot()[:,:,0] * RMFactor
    elapsed = (time.time()-start)
    print "Elapsed time to run Enzo RM, Vz projection = "+str(elapsed)
    return [RM, Vpar]
...
def _EnzoBparDensity(field, data):
    normal_vector = data.get_field_parameter('Normal_Vector')
    return normal_vector[0] * data["Bx"] * data["Density"] + normal_vector[1] * data["By"] * data["Density"] + normal_vector[2] * data["Bz"] * data["Density"]

def _EnzoVparFlux(field, data):
    normal_vector = data.get_field_parameter('Normal_Vector')
    ApecData = data.get_field_parameter('ApecData')
    TFudge = data.get_field_parameter('TFudge')
    logT = np.log10(data['Temperature'] * BulletConstants.DegreesK_to_keV * TFudge) * 100.0
    ...
    minT = logT.astype(int)
    flux = (minT+1.0-logT) * ApecData[minT+100,0] + (logT-minT) * ApecData[minT+101,0]
    Vpar = normal_vector[0] * data["x-velocity"] + normal_vector[1] * data["y-velocity"] + normal_vector[2] * data["z-velocity"]
    return Vpar * flux * data['Density']**2 / BulletConstants.mp**2 * 1E-14 
...
def BuildEnzoVelocityHistogram(pf, phi=0.0, theta = 0.0, psi = 0.0):  
    # This builds a velocity histogram for partciles from each of the two clusters

    start = time.time()
    R=EulerAngles(phi,theta,psi)
    CollisionAngle = 0.22 # This is the approximate angle of the collision to the coordinate angles
    mass=list()
    x=list()
    y=list()
    z=list()
    ...
    vperp=list()
    NPart = 0 # Particle counter	
    for i in range(pf.h.num_grids): # Read in all of the particle masses, positions, and velocities
	PixelVolume = pf.h.grids[i]['dx'] * pf.h.grids[i]['dy'] * pf.h.grids[i]['dz']
	for j in range(int(pf.h.grid_particle_count[i])):
		#if NPart%100000 == 0:
			#print '*'
		mass.append(pf.h.grids[i]['particle_mass'][j] * PixelVolume)	
     		...
     		y.append(XP[1])
   		z.append(XP[2])
                V = [pf.h.grids[i]['particle_velocity_x'][j],pf.h.grids[i]['particle_velocity_y'][j],pf.h.grids[i]['particle_velocity_z'][j]]
                VP = dot(R,V)
     		vpar.append(-VP[2] * 1.0E-5) # Put in km/sec - sign puts bullet moving away
     		vperp.append((VP[0] * cos(CollisionAngle) + VP[1] * sin(CollisionAngle)) * 1.0E-5) # Put in km/sec
		NPart = NPart + 1
    ...
    vmax = 10000.0
    nbin = 200
    Hist0 = Array1d(-vmax,vmax,nbin) # Histogram for binning the velocities
    Hist1 = Array1d(-vmax,vmax,nbin) # Histogram for binning the velocities
    Hist3 = Array1d(-vmax,vmax,nbin) # Histogram for binning the velocities
    Hist4 = Array1d(-vmax,vmax,nbin) # Histogram for binning the velocities
    VparHist = [Hist0,Hist1]
    VperpHist = [Hist3,Hist4]
    # Finding the 2 different masses
    Masses[0] = mass[0]
    #print 'Mass of particle type 0  = %.4f'%Masses[0]
    while (Masses[1] == 0):
    ...
    for n in range(NPart): # n cycles through the number of particles of this type
		...
		if abs(mass[n] - Masses[0]) < 1E-8:	
			...
			Centroids[0,1] = Centroids[0,1] + y[n]
			Centroids[0,2] = Centroids[0,2] + z[n]
                        ipar = min(nbin-1,max(0,int((vpar[n]+vmax)/Hist0.dx)))                        
                        iperp = min(nbin-1,max(0,int((vperp[n]+vmax)/Hist0.dx)))                        
                        VparHist[0].data[ipar] += 1
                        VperpHist[0].data[iperp] += 1
		else:	
			NumPart[1] = NumPart[1] + 1
			Centroids[1,0] = Centroids[1,0] + x[n]
			Centroids[1,1] = Centroids[1,1] + y[n]
			Centroids[1,2] = Centroids[1,2] + z[n]
                        ipar = min(nbin-1,max(0,int((vpar[n]+vmax)/Hist0.dx)))                        
                        iperp = min(nbin-1,max(0,int((vperp[n]+vmax)/Hist0.dx)))                        
                        VparHist[1].data[ipar] += 1
                        VperpHist[1].data[iperp] += 1
    ...
    if NumPart[0] > NumPart[1]: # Swap everything to put the bullet particles first
        ...
        Masses[1] = TempMass
        TempHist = VparHist[0]
        VparHist[0] = VparHist[1]
        VparHist[1] = TempHist
        TempHist = VperpHist[0]
        VperpHist[0] = VperpHist[1]
        VperpHist[1] = TempHist
        for m in range(3):
            TempCentroid = Centroids[0,m]
            Centroids[0,m] = Centroids[1,m]
            Centroids[1,m] = TempCentroid
    ...
    return [NumPart, Masses, Centroids, VparHist, VperpHist]
...
def BestPlot(snap,Z,phi=0.0,theta=0.0,psi=0.0,PlotSuffix='New_Kappa',FomLocate=False,TFudge=1.0,ConstrainPhi=False, Mask=(1,0,0,0,0,0),SpectralIndex=3.2,MaxShift=0.0):
	...
	try:
            ...
            simtime=simtime,fom=fom,line=[70,59,0.24],legend_location=[1.02, 1.0],line_plot_yticks=[0.0,25.0,50.0,75.0])
            pp.savefig(fig)
...
def MoviePlotsRMVz(snapmin,snapmax,xyarray,zmin,zmax,Z,phi=0.0,theta=0.0,psi=0.0,TFudge=1.0):
    # This creates a movie of a 2D slice through the snapshot file, using x, y limits,
    # from the input xyarray, with the slice having z extents from zmin to zmax.
    # This isn't really form moves, just snapshots
    # This plots RM and Vz maps
	...
	[RM, Vpar] = ProjectEnzoRMVz(pf,xyarray,phi=phi,theta=theta,psi=psi,zmin=zmin,zmax=zmax)
        PixelVolume=xyarray.dx * xyarray.dy * (zmax - zmin)
        ...
        VparMin = -1000.0
        VparMax = 1000.0
	print "RMMin = %g, RMMax = %g"%(RM.data.min(), RM.data.max())
	print "VparMin = %g, VparMax = %g"%(Vpar.data.min(), Vpar.data.max())

        RMHist = RM.data.flatten()
	HistRange=[1000.0,23000.0]
        [dyy,dxx] = meshgrid(xyarray.y,xyarray.x)# Data grid for plots
 
        figure()
        suptitle('Bullet Cluster Collision, T = %.2f Gy'%simtime, fontsize=16)
        ax1 = axes([0.25,0.45,0.5,0.45])
        n,bins,patches = ax1.hist(RMHist,100,range=HistRange, histtype='bar')
	ax1.set_xlabel('Rotation Measure (rads/m^2)')

        ax2 = axes([0.25,0.10,0.5,0.25],aspect=1.0)
	RMlevels=linspace(RMMin,RMMax,101) 
	for i in range(101): RMlevels[i]=int(RMlevels[i]*100)/100.0
        con2 = ax2.contourf(dxx,dyy,abs(RM.data),RMlevels,cmap=cm.spectral)
	for c in con2.collections:
    		c.set_linewidth(0.1)
    		c.set_alpha(1.0)
	ax2.set_xticks([-1000.0,0.0,1000.0])
	ax2.set_xlabel('kpc')
	ax2.set_yticks([-500.0,0.0,500.0])
	ax2.set_ylabel('kpc')
        c2 = colorbar(con2, pad = 0.02)

        filename = 'movie/collision_'+str('%03d'%snap)+'_RM.png'
	plt.savefig(filename)#, size=(2400,1800))
        print 'Wrote file', filename
        ...
        ax1 = axes([0.25,0.10,0.5,0.25],aspect=1.0)
	ax1.set_title('Flux Weighted Fluid Velocity (km/sec)')
	Vparlevels=linspace(VparMin,VparMax,101) 
	for i in range(101): Vparlevels[i]=int(Vparlevels[i]*100)/100.0
        con1 = ax1.contourf(dxx,dyy,Vpar.data,Vparlevels,cmap=cm.spectral)
	for c in con1.collections:
    		c.set_linewidth(0.1)
    		c.set_alpha(1.0)
	ax1.set_xticks([-1000.0,0.0,1000.0])
	ax1.set_xlabel('kpc')
	ax1.set_yticks([-500.0,0.0,500.0])
	ax1.set_ylabel('kpc')
        c1 = colorbar(con1, pad = 0.02)


        ax2 = axes([0.25,0.60,0.5,0.25],aspect=1.0)
	ax2.set_title('Flux Weighted Fluid Velocity (km/sec)')
        nxmin = Vpar.nx/4; nxmax = 3*Vpar.nx/4; nymin = Vpar.ny/4; nymax = 3*Vpar.ny/4; 
        con2 = ax2.contourf(dxx[nxmin:nxmax,nymin:nymax],dyy[nxmin:nxmax,nymin:nymax],Vpar.data[nxmin:nxmax,nymin:nymax],Vparlevels,cmap=cm.spectral)
	for c in con2.collections:
    		c.set_linewidth(0.1)
    		c.set_alpha(1.0)
	ax2.set_xticks([-500.0,0.0,500.0])
	ax2.set_xlabel('kpc')
	ax2.set_yticks([-250.0,0.0,250.0])
	ax2.set_ylabel('kpc')
        c2 = colorbar(con2, pad = 0.02)

        filename = 'movie/collision_'+str('%03d'%snap)+'_Vpar.png'
	plt.savefig(filename)#, size=(2400,1800))
        print 'Wrote file', filename
    
    return 

def DMHist(snapmin,snapmax,phi=0.0,theta=0.0,psi=0.0):
    # Creates DM velocity histograms
	...
	simtime = pf.h.parameters['InitialTime'] * BulletConstants.TimeConversion # Put time in Gyears
        ...
        [NumPart, Masses, Centroids, VparHist, VperpHist] = BuildEnzoVelocityHistogram(pf, phi, theta, psi)
        figure()
        #suptitle('Bullet Cluster Collision, T = %.2f Gy'%simtime, fontsize=16)
        ax1 = axes([0.25,0.08,0.5,0.15])
        ax1.bar(VperpHist[0].x,VperpHist[0].data)
        ax1.set_title('Bullet Cluster $V_{P}$ at T= %.2f'%simtime)
        ax1.set_xticks([-8000.0,-4000.0,0.0,4000.0,8000.0])
	ax1.set_xlabel('V (km/sec)')

        ax2 = axes([0.25,0.32,0.5,0.15])
        ax2.bar(VparHist[0].x,VparHist[0].data)
        ax2.set_title('Bullet Cluster $V_{LOS}$ at T= %.2f'%simtime)
        ax2.set_xticks([-8000.0,-4000.0,0.0,4000.0,8000.0])

        ax3 = axes([0.25,0.56,0.5,0.15])
        ax3.bar(VperpHist[1].x,VperpHist[1].data)
        ax3.set_title('Main Cluster $V_{P}$ at T= %.2f'%simtime)
        ax3.set_xticks([-8000.0,-4000.0,0.0,4000.0,8000.0])

        ax4 = axes([0.25,0.80,0.5,0.15])
        ax4.bar(VparHist[1].x,VparHist[1].data)
        ax4.set_title('Main Cluster $V_{LOS}$ at T= %.2f'%simtime)
        ax4.set_xticks([-8000.0,-4000.0,0.0,4000.0,8000.0])

        filename = 'movie/collision_'+str('%03d'%snap)+'_DMVHist.png'
	plt.savefig(filename)#, size=(2400,1800))
        print 'Wrote file', filename

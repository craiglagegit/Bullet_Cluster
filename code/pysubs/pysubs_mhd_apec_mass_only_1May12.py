...
def GridGadgetDM(snapfile,mass,parttype,phi=0.0,theta=0.0,psi=0.0): 
    ...
    print "Searched through %d particles.  A total of %d were in the grid, with a total mass of %f\n"%(num, ndm, mtot)
...
def FindFom3(snapmin,snapmax,Z,phi=3.14,theta=1.57,psi=0.0,ConstrainPsi=False):

	# This is a new, faster Fom finding routine that uses only mass and X-ray, and searches for a 
	# minimum in time, phi, and theta.
	# This version first scans through a Theta,Phi matrix to deterine the optimum.
	...
	DeltaPhi = 10 # Using Phi and Theta 100* phi and theta, so I can use integers
	DeltaTheta = 10 # Using DeltaPhi and Delta theta of 0.10 radians or about 6 degrees
	PhiMax = int(100*phi) + 30
  	PhiMin = int(100*phi) - 30
	ThetaMax = 314
	ThetaMin = 4
	NPhi = int((PhiMax-PhiMin)/DeltaPhi) + 3
	NTheta = int((ThetaMax-ThetaMin)/DeltaTheta) + 3
...
def BestPlot(snap,Z,phi=0.0,theta=0.0,psi=0.0,PlotSuffix='New_Kappa',FomLocate=False,IncludeTemp=False,ConstrainPsi=False):
	...
	if IncludeTemp:
		...
		masklist=list((maskA,maskANull,maskANull,maskANull,maskANull,maskDNull))

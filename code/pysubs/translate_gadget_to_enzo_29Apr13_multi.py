	...
	if i==0 or i==1:
		...
		if FieldMode == 'Random':
			...
			for nn in range(len(lines)):
				...
				jj = (nn % (nz*ny) - kk) / nz
				ii = (nn - jj*nz - kk) / (nz * ny)
				bx1.data[ii,jj,kk] = float(lines[nn].split()[0]) * MagScaleFactor * pow(SmoothedDensity[ii,jj,kk] / RhoMax, 1.0/3.0) * 1.0E-6 / EnzoBField
				by1.data[ii,jj,kk] = float(lines[nn].split()[1]) * MagScaleFactor * pow(SmoothedDensity[ii,jj,kk] / RhoMax, 1.0/3.0) * 1.0E-6 / EnzoBField
				bz1.data[ii,jj,kk] = float(lines[nn].split()[2]) * MagScaleFactor * pow(SmoothedDensity[ii,jj,kk] / RhoMax, 1.0/3.0) * 1.0E-6 / EnzoBField

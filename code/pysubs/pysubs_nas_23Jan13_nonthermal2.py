...
def ProjectEnzoData(pf,mass,phi=0.0,theta=0.0,psi=0.0,zmin=-3000.0,zmax=3000.0,DMProject=False):
    ...
    add_field('FTemperature', function = _FTemperature, validators=[ValidateParameter('TFudge')], take_log=False)
...
def _FTemperature(field, data):
    eta = data.get_field_parameter('TFudge')
    Omega2 = (data["Velocity_Vorticity1"] * data["Velocity_Vorticity1"] + data["Velocity_Vorticity2"] * data["Velocity_Vorticity2"] + data["Velocity_Vorticity3"] * data["Velocity_Vorticity3"])
    TR = 0.7 - eta * Omega2
    Teff = data["Temperature"] * TR 
    np.putmask(Teff,Teff<5.0E7,5.0E7)
    return Teff

def _EnzoTXRay(field,data):
	ApecData = data.get_field_parameter('ApecData')
	T = data['FTemperature'] * BulletConstants.DegreesK_to_keV
...
def _EnzoXRay(field,data):
	ApecData = data.get_field_parameter('ApecData')
	logT = np.log10(data['FTemperature'] * BulletConstants.DegreesK_to_keV) * 100.0
	np.putmask(logT,logT>179.0,179.0)
	np.putmask(logT,logT<-100.0,-100.0)
	minT = logT.astype(int)
	flux = (minT+1.0-logT) * ApecData[minT+100,3] + (logT-minT) * ApecData[minT+101,3]
...
def _EnzoSZ(field,data):
	T = data['FTemperature'] * BulletConstants.DegreesK_to_eV
...
def GetData():
	...
	datapathsA=toppath+'bullet/data/mass_sigma_23Jan13.dat'

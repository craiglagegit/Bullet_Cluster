...
divcleanlib=CDLL(lageconfig.bulletpath+'Div_Clean.so')
...
class CArray3d(Structure):
    # This is for calling c subroutines
    _fields_ = [("nx", c_int),("ny", c_int),("nz", c_int),("xmin",c_double),("xmax",c_double),("ymin",c_double),("ymax",c_double),("zmin",c_double),("zmax",c_double),("dx",c_double),("dy",c_double),("dz",c_double),("x",POINTER(c_double)),("y",POINTER(c_double)),("z",POINTER(c_double)),("data",POINTER(c_double))]
	...
	self.ymin=array.ymin
	self.ymax=array.ymax
	self.zmin=array.zmin
	self.zmax=array.zmax
	size=array.nx*array.ny*array.nz  
	self.data=(c_double*size)()
	self.x=(c_double*array.nx)()
	self.y=(c_double*array.ny)()
        self.z=(c_double*array.nz)()

	for k in range(array.nz):
		self.z[k]=array.z[k]
	...
	for i in range(array.nx):
		...
		for j in range(array.ny):
                    for k in range(array.nz):
			self.data[i+j*array.nx+k*array.nx*array.ny]=array.data[i,j,k]
...
def ProjectEnzoTemp(pf,mass,phi=0.0,theta=0.0,psi=0.0,zmin=-3000.0,zmax=3000.0,MHD=True):
    # Projects 3D Enzo data onto a 2D grid
...
def CleanDivergence(Bx, By, Bz):
    # This routine cleans the divergence from a B-field set of arrays.

    start = time.time()
    CBx = CArray3d(Bx) # Create C arrays
    CBy = CArray3d(By) # Create C arrays
    CBz = CArray3d(Bz) # Create C arrays

    divcleanlib.DivClean(byref(CBx),byref(CBy),byref(CBz))
    # c++ subroutine cleans divergence
    
    for i in range(Bx.nx):
	for j in range(Bx.ny):
            for k in range(Bx.nz):
                Bx.data[i,j,k]=CBx.data[i+j*Bx.nx+k*Bx.nx*Bx.ny]
                By.data[i,j,k]=CBy.data[i+j*Bx.nx+k*Bx.nx*Bx.ny]
                Bz.data[i,j,k]=CBz.data[i+j*Bx.nx+k*Bx.nx*Bx.ny]

    elapsed = (time.time()-start)
    print "Elapsed time to clean divergence = "+str(elapsed)+"\n"
...
def MoviePlots1(snapmin,snapmax,xyarray,zmin,zmax,Z,phi=0.0,theta=0.0,psi=0.0,TFudge=1.0):
    # This creates a movie of a 2D slice through the snapshot file, using x, y limits,
    # from the input xyarray, with the slice having z extents from zmin to zmax.
    # This adds dark matter and plots gas temperature, xray intensity, dm density and an overlay.
    # This version uses a 'bright' color scheme.
	...
	[tempsim]=ProjectEnzoTemp(pf,gasrho,phi=phi,theta=theta,psi=psi,zmin=zmin,zmax=zmax,MHD=False)
        PixelVolume=xyarray.dx * xyarray.dy * (zmax - zmin)
	DensityFactor = BulletConstants.cm_per_kpc**3 * PixelVolume / (BulletConstants.g_per_Msun * 1E10)
	gasrho.data = gasrho.data / DensityFactor
	#for i in range(100):

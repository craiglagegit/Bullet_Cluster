...
import  pysubs_nas_noB_17Mar14 as pysubs
...
if rank == 0:
	pp=PdfPages('plots/Graph_Profiles.pdf')
...
for snap in [0,10,39,59]:#[1,40,80]:#,97]:
	if snap == 0:
		ls = '-'
	elif snap == 10:
		ls = '--'
	...
	[TrueTemp,Temp] = pysubs.ProjectEnzoTemp(pf,mass,phi=0.0,theta=0.0,psi=0.0,zmin=zmin,zmax=zmax)	
	if rank == 0:
		...
		plot(mass.x-Centroids[1,1,0], (TrueTemp.data[:,0]+TrueTemp.data[:,1]+TrueTemp.data[:,2])/3.0 , label = 'Time = %.2f Gy'%simtime, ls = ls)
		xlim(-plotscale,plotscale)
		ylim(0.0,60.0)
		gca().set_xticks([-plotscale,0.0,plotscale])
	...
	[TrueTemp,Temp] = pysubs.ProjectEnzoTemp(pf,mass,phi=pi/2.0,theta=0.0,psi=0.0,zmin=zmin,zmax=zmax)	
	if rank == 0:
	...
	[TrueTemp,Temp] = pysubs.ProjectEnzoTemp(pf,mass,phi=pi/2.0,theta=pi/2.0,psi=0.0,zmin=zmin,zmax=zmax)	
	if rank == 0:

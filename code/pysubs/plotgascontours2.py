...
filenamebase = 'plots/'
pp=PdfPages('plots/Graph_Gas_Contours_Early.pdf')
xlimit = 800.0
zlimit = 100.0
xmin = ymin = -xlimit
xmax = ymax = xlimit
...
zmax = zlimit
counter = 0
levels = [0.1,0.178,0.316,0.562,1.0,1.78,3.16,5.62,10.0,17.8,31.6,56.2,100.0]#[10**-4.0,10**-3.5,10**-3.0,10**-2.5,-2.0]
figure()
figure()
subplots_adjust(hspace=0.8, wspace=0.4)
for snap in [0,16,30]:#[10,51,99]:
	...
	title('Gas Density, T=%.2f Gy'%simtime,fontsize=10)	
	contour(xx,yy,mass.data, levels,colors='b', linewidths=2.0)
	xlim(xmin,xmax)
	ylim(ymin,ymax)
	...
	title('Gas Density, T=%.2f Gy'%simtime,fontsize=10)	
	contour(xx,yy,mass.data, levels,colors='g', linewidths=2.0)
	xlim(xmin,xmax)
	ylim(ymin,ymax)
	gca().set_xticks([-xlimit/2,xlimit/2])
	gca().set_yticks([-xlimit/2,0.0,xlimit/2])
	...
	title('Gas Density, T=%.2f Gy'%simtime,fontsize=10)	
	contour(xx,yy,mass.data, levels,colors='r', linewidths=2.0)
	xlim(xmin,xmax)
	ylim(ymin,ymax)
	gca().set_xticks([-xlimit/2,xlimit/2])
	gca().set_yticks([-xlimit/2,0.0,xlimit/2])

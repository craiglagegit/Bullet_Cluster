...
/* Reworking this to create a 3D grid instead of projecting out a 2D grid.  Craig Lage - 21-Oct-10. */
  ...
  for(n = 0; n < NumPart; n++)
      ...
      if((n % (NumPart / 100)) == 0)
	{
      ...
      if(pos[0] + h < 0 || pos[0] - h >  LengthX || pos[1] + h < 0 || pos[1] - h > LengthY                                  || pos[2] + h < 0 || pos[2] - h > LengthZ)
        continue;/* Checks whether particle n can influence the region being gridded. */
#endif
          ...
          for(dz = -nz; dz <= nz; dz++)
            {
              ...
              //if (n==60149) printf("xx=%f,yy=%f,zz=%f,r2=%f\n",xx,yy,zz,r2);  
              if(r2 < h2)
                {
                  r = sqrt(r2);
                  u = r / h;
                  ...
                  //if (n==60149) printf("nx=%d,ny=%d,nz=%d,xx=%f,yy=%f,zz=%f,r=%f,u=%f,wk=%f\n",nx,ny,nz,xx,yy,zz,r,u,wk);              
                  sum += wk;
                }
            }
            ...
            for(dz = -nz; dz <= nz; dz++)
              {
				...
				InVol=wk/sum;
				printf("n= %d, Mass[n]=%f, Rho[n]=%f, h=%f, InVol=%f \n",n,Mass[n],Quantity[n],h,InVol);
				printf("counter=%d, sum=%f, wk=%f ,Hsml[n]=%f \n",counter, sum, wk,Hsml[n]);
				printf("px=%f, py=%f, pz=%f \n",P[n].Pos[Axis1],P[n].Pos[Axis2],P[n].Pos[Axis3]);
			        counter=counter+1;
                              }
                          }
                  }

...
This uses a Newton's method function minimization routine from Numerical Recipes to align three 2d functions and calculate a chi^2 figure of merit
...
{
  Arr *data1A, *shifteddata1A, *data2A, *sigmaA, *data1B, *shifteddata1B, *data2B, *sigmaB, *data1C, *shifteddata1C, *data2C, *sigmaC;
  Align *align;
  double EPS;
  double f; //Holds the most recent value of the function
  int evals; // Counts the number of function evaluations
  Func2(Arr *, Arr *, Arr *, Arr *, Arr *, Arr *, Arr *, Arr *, Arr *, Arr *, Arr *, Arr *, Align *); //Constructor
  double shift();
...
extern "C" int optimize3(Arr*, Arr*, Arr*, Arr*, Arr*, Arr*, Arr*, Arr*, Arr*, Arr*, Arr*, Arr*, Align*, double*, const double);

//THIS IS THE TOP LEVEL METHOD

extern "C" int optimize3(Arr* d1Aptr, Arr* sd1Aptr, Arr* d2Aptr, Arr* sAptr, Arr* d1Bptr, Arr* sd1Bptr, Arr* d2Bptr, Arr* sBptr, Arr* d1Cptr, Arr* sd1Cptr, Arr* d2Cptr, Arr* sCptr, Align* aptr, double* fptr, const double gtol)
// This is a wrapper function to translate the Python call
// to use the appropriate c++ optimization routines.

{
  Func2 func2(d1Aptr,sd1Aptr,d2Aptr,sAptr,d1Bptr,sd1Bptr,d2Bptr,sBptr,d1Cptr,sd1Cptr,d2Cptr,sCptr,aptr);
...
Func2::Func2(Arr* d1Aptr, Arr* sd1Aptr, Arr* d2Aptr, Arr* sAptr, Arr* d1Bptr, Arr* sd1Bptr, Arr* d2Bptr, Arr* sBptr, Arr* d1Cptr, Arr* sd1Cptr, Arr* d2Cptr, Arr* sCptr, Align* aptr)
// Func2 constructor
{
  evals=0;
  EPS=1.0E-4;
...

{
  ...
  for (i=0; i<data2A->nx; i++) 
      ...
      for (j=0; j<data2A->ny; j++)
	{
	  ...
	  fom = fom + pow((shifteddata1A->data[i+j*data2A->nx] - data2A->data[i+j*data2A->nx]) / sigmaA->data[i+j*data2A->nx],2);
	}
    }
  for (i=0; i<data2B->nx; i++) 
      ...
      for (j=0; j<data2B->ny; j++)
	{
	  ...
	  fom = fom + pow((shifteddata1B->data[i+j*data2B->nx] - data2B->data[i+j*data2B->nx]) / sigmaB->data[i+j*data2B->nx],2);
	}
    }
  for (i=0; i<data2C->nx; i++) 
    {
      x=data2C->x[i];
      for (j=0; j<data2C->ny; j++)
	{
	  y=data2C->y[j];
	  xprime=x*cos(align->d[2])+y*sin(align->d[2])+align->d[0];
	  yprime=-x*sin(align->d[2])+y*cos(align->d[2])+align->d[1];
	  shifteddata1C->data[i+j*data2C->nx]=interp(data1C,xprime,yprime);
	  fom = fom + pow((shifteddata1C->data[i+j*data2C->nx] - data2C->data[i+j*data2C->nx]) / sigmaC->data[i+j*data2C->nx],2);
	}
    }
  return fom;
}

...
import pysubs_nas_12Oct12 as pysubs
from pylab import *
...
def EnzoRunComplete(snap):
	...
	LastFileSize = 0
	if snap<10:
		EnzoWaitFile = "DD000"+str(snap)+"/output_000"+str(snap)
	elif snap<100:
		EnzoWaitFile = "DD00"+str(snap)+"/output_00"+str(snap)
	elif snap<1000:
		EnzoWaitFile = "DD0"+str(snap)+"/output_0"+str(snap)
	else:
		EnzoWaitFile = "DD"+str(snap)+"/output_"+str(snap)

	while NumTries < 4:
		try:
			NumTries = NumTries + 1
			FileSize = os.path.getsize(EnzoWaitFile)
...
for snap in range(snapmin, snapmax):
	if rank == 0:
		...
		if CenterSep < 500.0 or Centroids[0][0] < Centroids[1][0]: #Bullet hasn't yet reached 500 kpc past main
			SnapFlag = 'Wait'
		elif CenterSep > 1200.0 and Centroids[0][0] > Centroids[1][0]: #Bullet has passed main and sep > 1200 kpc means we're done
			SnapFlag = 'Break'
		else:
			SnapFlag = 'Run'
	else:
	...
	print "Processor #%d, snap = %d, SnapFlag = %s\n"%(rank,snap,SnapFlag)
	sys.stdout.flush()
	if SnapFlag == 'Break':
		break
	elif SnapFlag == 'Wait':
		continue
	# If SnapFlag == 'Run', then we start the angle runs, loggiing them in case we want to look later
	...
	if rank == 0:
		...
		print "Processor #%d, snap = %d, bestmassfom = %f, bestmasssnap = %d, bestmasstheta = %f, betsmasspsi = %f\n"%(rank,snap,bestmassfom, bestmasssnap, bestmasstheta, bestmasspsi)
		print "Processor #%d, snap = %d, bestmassx1fom = %f, bestmassx1snap = %d, bestmassx1theta = %f, betsmassx1psi = %f\n"%(rank,snap,bestmassx1fom, bestmassx1snap, bestmassx1theta, bestmassx1psi)
		sys.stdout.flush()
# This writes out a file and makes plots when we're all done
if rank == 0:
	...
	result="FOM=%.6f, BestSnap = %d, BestTime =  %.4f, BestPsi = %.3f, BestTheta = %.3f\n"%(bestmassx1fom,bestmassx1snap,bestmassx1time,bestmassx1psi,bestmassx1theta)
	PlotSuffix = 'MassX1'
	Mask=(1,1,0,0,0,0)

...
def FindFom(snapmin,snapmax,Z,phi,theta,psi,ConstrainPhi=True, Mask=(1,0,0,0,0,0,0)):
	...
	while counter < MaxCounter:
		...
		print "%d times through - Snap = %d, Psi = %.2f, Theta = %.2f"%(counter, snap, psi, theta)
                ...
                if abs(alpha) > 15.0:
			FomMatrix[snap,Psi,Theta] = (1.0E5,0.0) # If it's outside allowed angle to LOS, give it a large FOM
                        fom = 1.0E5
                        Phi = 0.0
                else:
                    try:
...
def MoviePlotsMHD1(snapmin,snapmax,xyarray,zmin,zmax,Z,phi=0.0,theta=0.0,psi=0.0):

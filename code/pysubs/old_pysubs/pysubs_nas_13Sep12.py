...
def ProjectEnzoData(pf,mass,phi=0.0,theta=0.0,psi=0.0,zmin=-3000.0,zmax=3000.0,DMProject=False):
	...
	print "Using yt Ray casting for DM"
        projcam=ProjectionCamera(center,normal_vector,width,resolution,"Dark_Matter_Density",north_vector=north_vector,pf=pf)
    ...
    SZ.data   = projcam.snapshot()[:,:,0] * SZFactor 
    elapsed = (time.time()-start)
...
def FindFom(dir,snapmin,snapmax,counter,Z,phi=0.0,theta=0.0,psi=0.0,PlotSuffix='Test',simulator="Gadget",PDF=True, FomLocate=False,IncludeTemp=True):
	...
	datapathsA=toppath+'bullet/data/mass_sigma_3May12.dat'
	...
	for snap in range(snapmin,snapmax): # Which snapshot files
		...
		if cos(phi)>=0.0:
			...
			align.d[1] = -95.0
			if ConstrainPsi:
				align.dmin[2] = 0.12  + arctan( cos(theta)*tan(phi) )
				align.dmax[2] = 0.32  + arctan( cos(theta)*tan(phi) )
				align.dmin[0] = align.dmin[1] = -110.0
				align.dmax[0] = align.dmax[1] = 10.0
		else:
			align.d[2] = 0.22  + pi + arctan( cos(theta)*tan(phi) ) # Seed psi close to final result
			align.d[0] = -40.0
			align.d[1] = -95.0
			if ConstrainPsi:
				align.dmin[2] = 0.12  + pi + arctan( cos(theta)*tan(phi) ) # Seed psi close to final result
				align.dmax[2] = 0.32  + pi + arctan( cos(theta)*tan(phi) ) # Seed psi close to final result
				align.dmin[0] = align.dmin[1] = -110.0
				align.dmax[0] = align.dmax[1] = 10.0
...
def FindFom2(snapmin,snapmax,Z,phi=0.0,theta=0.0,psi=0.0,NumAngles=8,ConstrainPsi=False):
	...
	PhiMax = int (100*phi) + NumAngles * DeltaPhi
  	PhiMin = int(100*phi) - NumAngles * DeltaPhi
	ThetaMax = int(100*theta) + NumAngles * DeltaTheta
	ThetaMin = int(100*theta) - NumAngles * DeltaTheta
	...
	while counter < MaxCounter:
		...
		FomMatTol = FomMatrix[snap,Phi,Theta] * 0.99
		if  FomMatTol  < FomMatrix[snap+1,Phi,Theta] and FomMatTol < FomMatrix[snap-1,Phi,Theta] \
		and FomMatTol < FomMatrix[snap,Phi+DeltaPhi,Theta] and FomMatTol < FomMatrix[snap,Phi-DeltaPhi,Theta] \
		and FomMatTol < FomMatrix[snap,Phi,Theta+DeltaTheta] and FomMatTol < FomMatrix[snap,Phi,Theta-DeltaTheta]:
			simtime = pf.h.parameters['InitialTime'] * BulletConstants.TimeConversion # Put time in Gyears
...
def BestPlot(snap,Z,phi=0.0,theta=0.0,psi=0.0,PlotSuffix='New_Kappa',FomLocate=False,IncludeTemp=False,ConstrainPsi=False):
	...
	sigmalist=list((sigmaA,sigmaB1,sigmaB2,sigmaB3,sigmaC,sigmaD))
	if IncludeTemp:
		masklist=list((maskA,maskANull,maskANull,maskANull,maskANull,maskD))
	else:
		masklist=list((maskA,maskA,maskANull,maskANull,maskANull,maskDNull))
...
def SimpleFom(snap,Z,phi=0.0,theta=0.0,psi=0.0,ConstrainPsi=False):
	...
	try:
		...
		return (fommassx1,fommass)
	except:
		return 1.0E5 # If there's an error, give it a large FOM

def MoviePlotsMHD1(snapmin,snapmax,xyarray,zmin,zmax,Z):

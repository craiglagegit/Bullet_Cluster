...
def CalculateEnzoSpectrum(pf,phi=0.0,theta=0.0,psi=0.0,zmin=-3000.0,zmax=3000.0):
    # Calculates the unresolved X-Ray spectrum

    mass = Array2d(zmin,zmax,10,zmin,zmax,10) 
    start = time.time()
    xpixels=mass.nx
    ypixels=mass.ny
    PixelArea = mass.dx * mass.dy
    ...
    Xray2=Array2d(mass.xmin,mass.xmax,mass.nx,mass.ymin,mass.ymax,mass.ny)
    Xray3=Array2d(mass.xmin,mass.xmax,mass.nx,mass.ymin,mass.ymax,mass.ny)
    Xray4=Array2d(mass.xmin,mass.xmax,mass.nx,mass.ymin,mass.ymax,mass.ny)
    Xray5=Array2d(mass.xmin,mass.xmax,mass.nx,mass.ymin,mass.ymax,mass.ny)
    Xray6=Array2d(mass.xmin,mass.xmax,mass.nx,mass.ymin,mass.ymax,mass.ny)
    Xray7=Array2d(mass.xmin,mass.xmax,mass.nx,mass.ymin,mass.ymax,mass.ny)

    add_field('XRay1', function = _EnzoXRay1, units=r"\rm{cm}^{-3}\rm{s}^{-1}", projected_units=r"\rm{cm}^{-2}\rm{s}^{-1}", validators=[ValidateParameter('ApecData')], take_log=False)
    add_field('XRay2', function = _EnzoXRay2, units=r"\rm{cm}^{-3}\rm{s}^{-1}", projected_units=r"\rm{cm}^{-2}\rm{s}^{-1}", validators=[ValidateParameter('ApecData')], take_log=False)
    add_field('XRay3', function = _EnzoXRay3, units=r"\rm{cm}^{-3}\rm{s}^{-1}", projected_units=r"\rm{cm}^{-2}\rm{s}^{-1}", validators=[ValidateParameter('ApecData')], take_log=False)
    add_field('XRay4', function = _EnzoXRay4, units=r"\rm{cm}^{-3}\rm{s}^{-1}", projected_units=r"\rm{cm}^{-2}\rm{s}^{-1}", validators=[ValidateParameter('ApecData')], take_log=False)
    add_field('XRay5', function = _EnzoXRay5, units=r"\rm{cm}^{-3}\rm{s}^{-1}", projected_units=r"\rm{cm}^{-2}\rm{s}^{-1}", validators=[ValidateParameter('ApecData')], take_log=False)
    add_field('XRay6', function = _EnzoXRay6, units=r"\rm{cm}^{-3}\rm{s}^{-1}", projected_units=r"\rm{cm}^{-2}\rm{s}^{-1}", validators=[ValidateParameter('ApecData')], take_log=False)
    add_field('XRay7', function = _EnzoXRay7, units=r"\rm{cm}^{-3}\rm{s}^{-1}", projected_units=r"\rm{cm}^{-2}\rm{s}^{-1}", validators=[ValidateParameter('ApecData')], take_log=False)

    pf.field_info['Density'].take_log=False
    pf.field_info['XRay1'].take_log=False
    pf.field_info['XRay2'].take_log=False
    pf.field_info['XRay3'].take_log=False
    pf.field_info['XRay4'].take_log=False
    pf.field_info['XRay5'].take_log=False
    pf.field_info['XRay6'].take_log=False
    pf.field_info['XRay7'].take_log=False
    ...
    projcam=ProjectionCamera(center,normal_vector,width,resolution,"XRay3",north_vector=north_vector,pf=pf, interpolated=True)
    Xray3.data = projcam.snapshot()[:,:,0] * XFactor
    projcam=ProjectionCamera(center,normal_vector,width,resolution,"XRay4",north_vector=north_vector,pf=pf, interpolated=True)
    Xray4.data = projcam.snapshot()[:,:,0] * XFactor
    projcam=ProjectionCamera(center,normal_vector,width,resolution,"XRay5",north_vector=north_vector,pf=pf, interpolated=True)
    Xray5.data = projcam.snapshot()[:,:,0] * XFactor
    projcam=ProjectionCamera(center,normal_vector,width,resolution,"XRay6",north_vector=north_vector,pf=pf, interpolated=True)
    Xray6.data = projcam.snapshot()[:,:,0] * XFactor
    projcam=ProjectionCamera(center,normal_vector,width,resolution,"XRay7",north_vector=north_vector,pf=pf, interpolated=True)
    Xray7.data = projcam.snapshot()[:,:,0] * XFactor

    Spectrum = [Xray1.data.sum(),Xray2.data.sum(),Xray3.data.sum(),Xray4.data.sum(),Xray5.data.sum(),Xray6.data.sum(),Xray7.data.sum()]
...
def _EnzoXRay4(field,data):
	...
	np.putmask(logT,logT<-100.0,-100.0)
	minT = logT.astype(int)
	flux = (minT+1.0-logT) * ApecData[minT+100,4] + (logT-minT) * ApecData[minT+101,4]
	return flux * data['Density']**2 / BulletConstants.mp**2 * 1E-14

def _EnzoXRay5(field,data):
	ApecData = data.get_field_parameter('ApecData')
	TFudge = data.get_field_parameter('TFudge')
	logT = np.log10(data['Temperature'] * BulletConstants.DegreesK_to_keV * TFudge) * 100.0
	np.putmask(logT,logT>179.0,179.0)
	np.putmask(logT,logT<-100.0,-100.0)
	minT = logT.astype(int)
	flux = (minT+1.0-logT) * ApecData[minT+100,5] + (logT-minT) * ApecData[minT+101,5]
	return flux * data['Density']**2 / BulletConstants.mp**2 * 1E-14

def _EnzoXRay6(field,data):
	ApecData = data.get_field_parameter('ApecData')
	TFudge = data.get_field_parameter('TFudge')
	logT = np.log10(data['Temperature'] * BulletConstants.DegreesK_to_keV * TFudge) * 100.0
	np.putmask(logT,logT>179.0,179.0)
	np.putmask(logT,logT<-100.0,-100.0)
	minT = logT.astype(int)
	flux = (minT+1.0-logT) * ApecData[minT+100,6] + (logT-minT) * ApecData[minT+101,6]
	return flux * data['Density']**2 / BulletConstants.mp**2 * 1E-14

def _EnzoXRay7(field,data):
	ApecData = data.get_field_parameter('ApecData')
	TFudge = data.get_field_parameter('TFudge')
	logT = np.log10(data['Temperature'] * BulletConstants.DegreesK_to_keV * TFudge) * 100.0
	np.putmask(logT,logT>179.0,179.0)
	np.putmask(logT,logT<-100.0,-100.0)
	minT = logT.astype(int)
	flux = (minT+1.0-logT) * ApecData[minT+100,7] + (logT-minT) * ApecData[minT+101,7]
	return flux * data['Density']**2 / BulletConstants.mp**2 * 1E-14
...
def ReadLookups(Z):
	...
	ApecData = zeros([281,8]) # Array to hold the data
	for bin in range(8): #bin 0 is 0.5-2kev, bin 1 is 2-5kev, bin 2 is 5-8kev, bin 3 is 0.5-8kev
		if bin == 0:
			...
			infile = open(lageconfig.toppath+'bullet/data/apec/apec_xray_14.0_20.0.txt','r')
		elif bin == 5:
			infile = open(lageconfig.toppath+'bullet/data/apec/apec_xray_20.0_24.0.txt','r')
		elif bin == 6:
			infile = open(lageconfig.toppath+'bullet/data/apec/apec_xray_24.0_35.0.txt','r')
		elif bin == 7:
			infile = open(lageconfig.toppath+'bullet/data/apec/apec_xray_35.0_50.0.txt','r')
                if bin != 3:
...
def FindSpectrum(snapmin,snapmax,Z,TFudge=1.0):

	# Calculates the 1D X-Ray spectrum
        ...
        for snap in range(snapmin, snapmax):
            ...
            Spectrum = CalculateEnzoSpectrum(pf)

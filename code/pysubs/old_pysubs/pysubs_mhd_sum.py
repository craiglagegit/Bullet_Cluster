...
def FindFom(toppath,dir,snapmin,snapmax,counter,phi=0.0,theta=0.0,psi=0.0,PlotSuffix='Test',simulator="Gadget",PDF=True, FomLocate=False,IncludeTemp=True,Summary=False):
	...
	for snap in range(snapmin,snapmax): # Which snapshot files
		...
		if PlotSuffix != None and not Summary:
			try:
		...
		if Summary:
			try:
			    ...
			    contour(dxxA,dyyA,shiftedmsim.data, colors='k')
			    plot(dataslice.x,dataslice.y,'w:')# Plot the 1D slice
			    gca().set_xticks([-800.0,0.0,800.0])
			    xlabel('kpc')
			    ylabel('kpc')

			    subplot(2,3,4)
			    title('1D Slice through data max', fontsize=12)
			    plot(dataslice.x,dataslice.data,label="Data",linewidth=3.0)
			    plot(simslice.x,simslice.data,label="Sim",linewidth=3.0)
			    ...
			    subplot(2,3,5)
			    title('1D Slice through data max', fontsize=12)
			    plot(dataslice.x,dataslice.data*1.0E6,label="Data",linewidth=2.0)
			    plot(simslice.x,simslice.data*1.0E6,label="Sim",linewidth=2.0)

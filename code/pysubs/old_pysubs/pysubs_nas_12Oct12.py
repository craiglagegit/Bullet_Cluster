...
def FindFom(snapmin,snapmax,Z,phi,theta,psi,ConstrainPhi=True, Mask=(1,0,0,0,0,0,0)):
	...
	PsiMax = int(100*psi) + 15
  	PsiMin = int(100*psi) - 15
	ThetaMax = int(100*theta) + 15
	ThetaMin = int(100*theta) - 15
	...
	for snap in range(snapmin-1,snapmax+2): # First fill the whole array with 1E6 - this IDs the boundaries
		for Psi in range(PsiMin-DeltaPsi, PsiMax+2*DeltaPsi, DeltaPsi):	
			for Theta in range(ThetaMin-DeltaTheta, ThetaMax+2*DeltaTheta, DeltaTheta):
				FomMatrix[snap,Psi,Theta] = 1000000.0				
	...
	while counter < MaxCounter:
		...
		print "%d times through - Snap = %d, Psi = %.2f, Theta = %.2f"%(counter, snap, psi, theta)
                ...
                if abs(alpha) > 15.0:
			fom = FomMatrix[snap,Psi,Theta] = 1.0E5 # If it's outside allowed angle to LOS, give it a large FOM
                else:
                    try:
			pf = GetPF(snap)
			...
			FomMatrix[snap,Psi,Theta] = fom
                    except:
			fom = FomMatrix[snap,Psi,Theta] = 1.0E5 # If there's an error, give it a large FOM
		...
		for sn in range(snapmin-1,snapmax+1): 
			for Ps in range(PsiMin-DeltaPsi, PsiMax+DeltaPsi, DeltaPsi):	
				for Th in range(ThetaMin-DeltaTheta, ThetaMax+DeltaTheta, DeltaTheta):
					if FomMatrix[sn,Ps,Th] < fom and FomMatrix[sn,Ps,Th] > 0:
						fom = FomMatrix[sn,Ps,Th]
						snap = sn
						Psi = Ps
						Theta = Th
		# If this point is better than all points around it (within tolerance), we're done
		FomMatTol = FomMatrix[snap,Psi,Theta] * 0.99
		if  FomMatTol  < FomMatrix[snap+1,Psi,Theta] and FomMatTol < FomMatrix[snap-1,Psi,Theta] \
		and FomMatTol < FomMatrix[snap,Psi+DeltaPsi,Theta] and FomMatTol < FomMatrix[snap,Psi-DeltaPsi,Theta] \
		and FomMatTol < FomMatrix[snap,Psi,Theta+DeltaTheta] and FomMatTol < FomMatrix[snap,Psi,Theta-DeltaTheta]:
			simtime = pf.h.parameters['InitialTime'] * BulletConstants.TimeConversion # Put time in Gyears
			return (fom,snap,Psi/100.0,Theta/100.0,simtime,counter)
		...
		for (sn,Ps,Th) in [(snap-1,Psi,Theta),(snap+1,Psi,Theta),(snap,Psi+DeltaPsi,Theta),(snap,Psi-DeltaPsi,Theta),\
			...
			if FomMatrix[sn,Ps,Th] > 999.0: # This means it's a boundary
				continue
			elif FomMatrix[sn,Ps,Th] < 0.0: # This means it hasn't yet been run, so move there and run it.
				snap = sn
				Psi = Ps
				Theta = Th
...
def BestPlot(snap,Z,phi=0.0,theta=0.0,psi=0.0,PlotSuffix='New_Kappa',FomLocate=False,ConstrainPhi=False, Mask=(1,0,0,0,0,0)):
...
def SimpleFom(snap,Z,phi=0.0,theta=0.0,psi=0.0,ConstrainPhi=False):

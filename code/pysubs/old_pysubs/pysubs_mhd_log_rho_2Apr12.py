...
def ProjectEnzoData(pf,mass,phi=0.0,theta=0.0,psi=0.0,zmin=-3000.0,zmax=3000.0,DMProject=False):
    ...
    vp1 = pf.h.volume_rendering(normal_vector, width, center, resolution, tf, fields=['Density','SZ','Dark_Matter_Density'], north_vector=north_vector)
    vp1.ray_cast()
	...
	print "Using yt Ray casting for DM"
        DM.data =  vp1.image[:,:,2] * MassFactor

    mass.data =  vp1.image[:,:,0] * MassFactor
...
def _EnzoTXRay(field,data):
	...
	T = data['Temperature'] * BulletConstants.DegreesK_to_keV
...
def _EnzoXRay(field,data):
	ApecData = data.get_field_parameter('ApecData')
	logT = np.log10(data['Temperature'] * BulletConstants.DegreesK_to_keV) * 100.0
	np.putmask(logT,logT>179.0,179.0)
	np.putmask(logT,logT<-100.0,-100.0)
	minT = logT.astype(int)
	flux = (minT+1.0-logT) * ApecData[minT+100,3] + (logT-minT) * ApecData[minT+101,3]
...
def FindFom(dir,snapmin,snapmax,counter,Z,phi=0.0,theta=0.0,psi=0.0,PlotSuffix='Test',simulator="Gadget",PDF=True, FomLocate=False,IncludeTemp=True):
	...
	if IncludeTemp:
		...
		datapathmD=toppath+'bullet/data/xtemp_mask_null_29Jun11.dat' # Ignore temp in this case.
	...
	for snap in range(snapmin,snapmax): # Which snapshot files
		...
		if PlotSuffix != None:
			try:
			    ...
			    [dataslice,simslice] = OneDSlice2Fixed(dataA,shiftedmsim,0.24,70,53)
			    ...
			    subplot("224")
			    title('1D Slice through data max')
			    plot(dataslice.x,log10(dataslice.data),label="Data",linewidth=3.0)
			    plot(simslice.x,log10(simslice.data),label="Sim",linewidth=3.0)
...
def FindFom2(snapmin,snapmax,Z,phi=0.0,theta=0.0,psi=0.0):
	...
	DeltaPhi = 10 # Using Phi and Theta 100* phi and theta, so I can use integers
	DeltaTheta = 10 # Using DeltaPhi and Delta theta of 0.10 radians or about 6 degrees
	PhiMax = int (100*phi) + 4 * DeltaPhi
  	PhiMin = int(100*phi) - 4 * DeltaPhi
	ThetaMax = int(100*theta) + 4 * DeltaTheta
	ThetaMin = int(100*theta) - 4 * DeltaTheta
	NPhi = int((PhiMax-PhiMin)/DeltaPhi) + 3
	...
	for snap in range(snapmin-1,snapmax+2): # First fill the whole array with 1E6 - this IDs the boundaries
		for Phi in range(PhiMin-DeltaPhi, PhiMax+2*DeltaPhi, DeltaPhi):	
			for Theta in range(ThetaMin-DeltaTheta, ThetaMax+2*DeltaTheta, DeltaTheta):
				FomMatrix[snap,Phi,Theta] = 1000000.0				
	for snap in range(snapmin,snapmax+1): # Next put -1.0 everywhere except at the boundaries - this IDs conditions not yet run
		for Phi in range(PhiMin, PhiMax+DeltaPhi, DeltaPhi):	
			for Theta in range(ThetaMin, ThetaMax+DeltaTheta, DeltaTheta):
				FomMatrix[snap,Phi,Theta] = -1.0				
	snap = snapmin
	Phi = int(100*phi)
	Theta = int(100*theta)
	counter = 0
	MaxCounter = (snapmax-snapmin+1) * NPhi * NTheta
	TimeStripeDone = False
	while counter < MaxCounter:
		phi = Phi / 100.0
		theta = Theta / 100.0
		counter = counter + 1
		print "%d times through - Snap = %d, Phi = %.2f, Theta = %.2f"%(counter, snap, phi, theta)
		try:
			pf = GetPF(snap)
			dmsim=Array2d(-sxmaxA,sxmaxA,sxpixelsA,-symaxA,symaxA,sypixelsA)
			masssim=Array2d(-sxmaxA,sxmaxA,sxpixelsA,-symaxA,symaxA,sypixelsA)
			...
			align.d[0]=align.d[1]=0.0
			tol=1.0E-4 # Alignment tolerance
			data1list=list((sumsim,xraysim1,xraysim2,xraysim3))
			data2list=list((dataA,dataB1,dataB2,dataB3))
			sigmalist=list((sigmaA,sigmaB1,sigmaB2,sigmaB3))
			masklist=list((maskA,maskA,maskA,maskA))

			[shifteddata1list,align,fom]=FindBestShift(data1list, data2list, sigmalist, masklist, align, tol)

			FomMatrix[snap,Phi,Theta] = fom
		except:
			FomMatrix[snap,Phi,Theta] = 1.0E5 # If there's an error, give it a large FOM

	
		if snap == snapmax:
			TimeStripeDone = True

		if not TimeStripeDone: # First, complete one stripe through time from snapmin to snapmax
			snap = snap + 1
			continue

		# Then, find the best fom so far and go to this point:
		for sn in range(snapmin-1,snapmax+1): 
			for Ph in range(PhiMin-DeltaPhi, PhiMax+DeltaPhi, DeltaPhi):	
				for Th in range(ThetaMin-DeltaTheta, ThetaMax+DeltaTheta, DeltaTheta):
					if FomMatrix[sn,Ph,Th] < fom and FomMatrix[sn,Ph,Th] > 0:
						fom = FomMatrix[sn,Ph,Th]
						snap = sn
						Phi = Ph
						Theta = Th	

		# If this point is better than all points around it, we're done
		if  FomMatrix[snap,Phi,Theta] < FomMatrix[snap+1,Phi,Theta] and FomMatrix[snap,Phi,Theta] < FomMatrix[snap-1,Phi,Theta] \
		and FomMatrix[snap,Phi,Theta] < FomMatrix[snap,Phi+DeltaPhi,Theta] and FomMatrix[snap,Phi,Theta] < FomMatrix[snap,Phi-DeltaPhi,Theta] \
		and FomMatrix[snap,Phi,Theta] < FomMatrix[snap,Phi,Theta+DeltaTheta] and FomMatrix[snap,Phi,Theta] < FomMatrix[snap,Phi,Theta-DeltaTheta]:
			simtime = pf.h.parameters['InitialTime'] * BulletConstants.TimeConversion # Put time in Gyears
			return (fom,snap,Phi/100.0,Theta/100.0,simtime,counter)

		# If not, find a surrounding point not yet done and run it.
		for (sn,Ph,Th) in [(snap-1,Phi,Theta),(snap+1,Phi,Theta),(snap,Phi+DeltaPhi,Theta),(snap,Phi-DeltaPhi,Theta),\
				(snap,Phi,Theta+DeltaTheta),(snap,Phi,Theta-DeltaTheta)]:
			if FomMatrix[sn,Ph,Th] > 999.0: # This means it's a boundary
				continue
			elif FomMatrix[sn,Ph,Th] < 0.0: # This means it hasn't yet been run, so move there and run it.
				snap = sn
				Phi = Ph
				Theta = Th
				print "New snap = %d, New Phi = %.2f, New Theta = %.2f"%(snap,Phi,Theta)
				break
		
	return (1.0E5,0,0.0,0.0,0.0,0) # Returns garbage if it fails to find an optimum
...
def BestPlot(snap,Z,phi=0.0,theta=0.0,psi=0.0,PlotSuffix='Test',FomLocate=False,IncludeTemp=False):
	...
	# but the Enzo simulations have an additional pi/2 rotation.
	align.d[0]=align.d[1]=0.0
	#align.dmin[2] = 0.21  + arctan( cos(theta)*tan(phi) ) # Seed psi close to final result
	#align.dmax[2] = 0.23  + arctan( cos(theta)*tan(phi) ) # Seed psi close to final result
	...
	try:
	    ...
	    gca().set_xticks([-800.0,-400.0,0.0,800.0,400.0])
	    colorbar()
	    dataedge=Array2d(-dxmaxB,dxmaxB,xpixelsB,-dymaxB,dymaxB,ypixelsB)
	    simedge=Array2d(-dxmaxB,dxmaxB,xpixelsB,-dymaxB,dymaxB,ypixelsB)
	    dataBtrunc=Array2d(-dxmaxB,dxmaxB,xpixelsB,-dymaxB,dymaxB,ypixelsB)
	    simtrunc=Array2d(-dxmaxB,dxmaxB,xpixelsB,-dymaxB,dymaxB,ypixelsB)
		...
		for jj in range(dataB1.ny):
			...
			simtrunc.data[ii,jj] = max(min(xraymax, shiftedxsim1.data[ii,jj]),xraymin)
	    ...
	    subplot("223",aspect=1)
	    title("Sim Edge Detection - 500-2000eV")
	    contourf(dxxB,dyyB,simedge.data,cmap=cm.spectral)
	    for c in contourf(dxxB,dyyB,simedge.data,cmap=cm.spectral).collections:
		c.set_linewidth(0.1)
		c.set_alpha(1.0)
	    colorbar()
	    gca().set_xticks([-800.0,-400.0,0.0,800.0,400.0])
		...
		c.set_linewidth(0.1)
		c.set_alpha(1.0)
	    contour(dxxB,dyyB,simedge.data,linestyle='-',colors='k')
	    gca().set_xticks([-800.0,-400.0,0.0,800.0,400.0])
	    xlabel('kpc')
	    ylabel('kpc')
	    pp.savefig()
	    ...
	    # Temperature edge detection plots 

	    figure()
	    suptitle('Bullet Cluster Collision, T = %.2f Gy'%simtime, fontsize=16)
	    subplots_adjust(hspace=0.4, wspace=0.4)
	    tempdataedge=Array2d(-dxmaxD,dxmaxD,xpixelsD,-dymaxD,dymaxD,ypixelsD)

	    tempdataedge.data = hypot(sobel(shiftedtempsim.data, 0),sobel(shiftedtempsim.data, 1))

	    subplot("111",aspect=1)
	    title("Data Edge Detection - Temp")
	    contourf(dxxD,dyyD,tempdataedge.data,cmap=cm.spectral)
	    for c in contourf(dxxD,dyyD,tempdataedge.data,cmap=cm.spectral).collections:
		c.set_linewidth(0.1)
		c.set_alpha(1.0)
	    colorbar()
	    gca().set_xticks([-800.0,-400.0,0.0,800.0,400.0])
...
def MoviePlotShock(snapmin,snapmax,Z):
	xscaleA=yscaleA=BulletConstants.AngularScale*3600*BulletConstants.MassDataScale 
	# Pixel size in kpc. 4.413 kpc/" is the angular scale at the bullet cluster
	# 3600 "/degree, 9.86E-4 degrees/pixel is the data scale
	...
	theta = 2.35
	psi = pi
	TempEdgeLevels=linspace(0,50.0,25)
	MassEdgeLevels=linspace(0,5.0,25)
	...
	for snap in range(snapmin,snapmax): # Which snapshot files
		...
		try:
			...
			[syyA,sxxA] = meshgrid(masssim.y,masssim.x) # Sim grid for plots
			[syyD,sxxD] = meshgrid(tempmass.y,tempmass.x) # Sim grid for plots
			...
			[tempsim,bmagsim]=ProjectEnzoTemp(pf,tempmass,phi,theta,psi)
			masssimedge=Array2d(-sxmaxA,sxmaxA,sxpixelsA,-symaxA,symaxA,sypixelsA)
			tempsimedge=Array2d(-sxmaxD,sxmaxD,sxpixelsD,-symaxD,symaxD,sypixelsD)
			xraysimedge=Array2d(-sxmaxA,sxmaxA,sxpixelsA,-symaxA,symaxA,sypixelsA)
			"""
			xraymax = pow(10, -6.5)
			xraymin = pow(10, -8.0)
			for ii in range(xraysim1.nx):
				for jj in range(xraysim1.ny):
					xraysim1.data[ii,jj] = max(min(xraymax, xraysim1.data[ii,jj]),xraymin)

			xraysimedge.data = hypot(sobel(xraysim1.data, 0),sobel(xraysim1.data, 1))
			"""

			masssimedge.data = hypot(sobel(masssim.data, 0),sobel(masssim.data, 1))
			tempsimedge.data = hypot(sobel(tempsim.data, 0),sobel(tempsim.data, 1))
			...
			title("Edge Detection - Temp")
			contourf(sxxD,syyD,tempsimedge.data,TempEdgeLevels,cmap=cm.spectral)
			for c in contourf(sxxD,syyD,tempsimedge.data,TempEdgeLevels,cmap=cm.spectral).collections:
				c.set_linewidth(0.1)
				c.set_alpha(1.0)
			colorbar()
			gca().set_xticks([-1500.0,-1000.0,-500.0,0.0,500.0,1000.0,1500.0])
			xlabel('kpc')
			ylabel('kpc')

			subplot("212",aspect=1)
			title("Edge Detection - Density")
			contourf(sxxA,syyA,masssimedge.data,MassEdgeLevels,cmap=cm.spectral)
			for c in contourf(sxxA,syyA,masssimedge.data,MassEdgeLevels,cmap=cm.spectral).collections:
				c.set_linewidth(0.1)
				c.set_alpha(1.0)
			colorbar()
			gca().set_xticks([-1500.0,-1000.0,-500.0,0.0,500.0,1000.0,1500.0])
			...
			ylabel('kpc')
			"""
			subplot("313",aspect=1)
			title("Sim Edge Detection - XRay")
			contourf(sxxA,syyA,xraysimedge.data,cmap=cm.spectral)
			for c in contourf(sxxA,syyA,xraysimedge.data,cmap=cm.spectral).collections:
				c.set_linewidth(0.1)
				c.set_alpha(1.0)
			colorbar()
			gca().set_xticks([-1500.0,-1000.0,-500.0,0.0,500.0,1000.0,1500.0])

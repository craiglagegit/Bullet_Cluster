...
def FindFom(dir,snapmin,snapmax,counter,Z,phi=0.0,theta=0.0,psi=0.0,PlotSuffix='Test',simulator="Gadget",PDF=True, FomLocate=False,IncludeTemp=True):
	...
	datapathsA=toppath+'bullet/data/mass_sigma_25Apr12.dat'
	...
	for snap in range(snapmin,snapmax): # Which snapshot files
		...
		if cos(phi)>=0.0:
			...
			align.dmin[2] = 0.20  + arctan( cos(theta)*tan(phi) )
			align.dmax[2] = 0.24  + arctan( cos(theta)*tan(phi) )
		else:
			align.d[2] = 0.22  + pi + arctan( cos(theta)*tan(phi) ) # Seed psi close to final result
			align.dmin[2] = 0.20  + pi + arctan( cos(theta)*tan(phi) ) # Seed psi close to final result
			align.dmax[2] = 0.24  + pi + arctan( cos(theta)*tan(phi) ) # Seed psi close to final result
...
def FindFom2(snapmin,snapmax,Z,phi=0.0,theta=0.0,psi=0.0,NumAngles=4,ConstrainPsi=False):
		...
		try:
			...
			if cos(phi)>=0.0:
				...
				if ConstrainPsi:
					...
					align.dmin[0] = align.dmin[1] = -100.0
					align.dmax[0] = align.dmax[1] = 100.0
			else:
				align.d[2] = 0.22  + pi + arctan( cos(theta)*tan(phi) ) # Seed psi close to final result
				if ConstrainPsi:

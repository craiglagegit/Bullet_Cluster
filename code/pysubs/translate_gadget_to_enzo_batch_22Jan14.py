	...
	if i==0:
		...
		if FieldMode == 'Random':
			...
			for nn in range(len(lines)):
				...
				jj = (nn % (nz*ny) - kk) / nz
				ii = (nn - jj*nz - kk) / (nz * ny)
				bx1.data[ii,jj,kk] = float(lines[nn].split()[0]) * MagScaleFactor * pow(SmoothedDensity[ii,jj,kk] / RhoMax, 2.0/3.0) * 1.0E-6 / EnzoBField
				by1.data[ii,jj,kk] = float(lines[nn].split()[1]) * MagScaleFactor * pow(SmoothedDensity[ii,jj,kk] / RhoMax, 2.0/3.0) * 1.0E-6 / EnzoBField
				bz1.data[ii,jj,kk] = float(lines[nn].split()[2]) * MagScaleFactor * pow(SmoothedDensity[ii,jj,kk] / RhoMax, 2.0/3.0) * 1.0E-6 / EnzoBField
			...
			for ii in range(nx):
				for jj in range(ny):
					for kk in range(nz):
						...
						"""
		b2 = pysubs.Array3d(xmin,xmax,nx,ymin,ymax,ny,zmin,zmax,nz)	
		[dzz,dxx] = meshgrid(b2.z,b2.x)# Data grid for plots
		[filled_levels, line_levels] = pysubs.SetContourLevels(0.0, 25.0)
		for ii in range(nx):
			for jj in range(ny):
				for kk in range(nz):
					b2.data[ii,jj,kk] = sqrt(bx1.data[ii,jj,kk]*bx1.data[ii,jj,kk]+by1.data[ii,jj,kk]*by1.data[ii,jj,kk]+bz1.data[ii,jj,kk]*bz1.data[ii,jj,kk])
		print "Pre-clean, BMag max = %.4g, BMag min = %.4g"%(b2.data.max(), b2.data.min())					
		"""
		print "Pre-clean, Bx max = %.4g, Bx min = %.4g"%(bx1.data.max(), bx1.data.min())
		print "Pre-clean, By max = %.4g, By min = %.4g"%(by1.data.max(), by1.data.min())
		print "Pre-clean, Bz max = %.4g, Bz min = %.4g"%(bz1.data.max(), bz1.data.min())
		"""
		subplot(2,1,1)
		contourf(dxx, dzz, b2.data[:,ny/2,:],filled_levels)
		[bx1, by1, bz1] = pysubs.CleanDivergence(bx1,by1,bz1) # Clean the divergence after scaling
		for ii in range(nx):
			for jj in range(ny):
				for kk in range(nz):
					b2.data[ii,jj,kk] = sqrt(bx1.data[ii,jj,kk]*bx1.data[ii,jj,kk]+by1.data[ii,jj,kk]*by1.data[ii,jj,kk]+bz1.data[ii,jj,kk]*bz1.data[ii,jj,kk])
		print "Post-clean, BMag max = %.4g, BMag min = %.4g"%(b2.data.max(), b2.data.min())					
		"""
		print "Post-clean, Bx max = %.4g, Bx min = %.4g"%(bx1.data.max(), bx1.data.min())
		print "Post-clean, By max = %.4g, By min = %.4g"%(by1.data.max(), by1.data.min())
		print "Post-clean, Bz max = %.4g, Bz min = %.4g"%(bz1.data.max(), bz1.data.min())
		"""
		subplot(2,1,2)
		contourf(dxx, dzz, b2.data[:,ny/2,:],filled_levels)
		show()

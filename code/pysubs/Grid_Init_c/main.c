...
void findGridInit(int numpart, struct particle_data *p, float *hsml, float *mass, float *quantity1, float *quantity2, float *quantity3, float *quantity4, float xmin, float xmax, float ymin, float ymax, float zmin, float zmax, int xpixels, int ypixels, int zpixels, int desdensngb, int axis1, int axis2, int axis3, float hmax, double boxsize, float *value, float *valuequantity1, float *valuequantity2, float *valuequantity3, float *valuequantity4)
{
...
/* Adding additional quantity variables to grid out velocity Craig Lage 10-Jan-11. */
  ...
  Value =         value;
  ValueQuantity1 = valuequantity1;
  ValueQuantity2 = valuequantity2;
  ValueQuantity3 = valuequantity3;
  ValueQuantity4 = valuequantity4;
...
void make_map(void)
{
      ...
      for(k = 0; k < Zpixels; k++)
        {
          ...
          ValueQuantity1[i + Xpixels * j + Xpixels * Ypixels * k] = 0;
          ValueQuantity2[i + Xpixels * j + Xpixels * Ypixels * k] = 0;
          ValueQuantity3[i + Xpixels * j + Xpixels * Ypixels * k] = 0;
          ValueQuantity4[i + Xpixels * j + Xpixels * Ypixels * k] = 0;
        }
                            ...
                            if(r2 < h2)
                              {
                                ...
                                ValueQuantity1[i + Xpixels * j + Xpixels * Ypixels * k] += Mass[n]*Quantity1[n]*wk / sum;
                                ValueQuantity2[i + Xpixels * j + Xpixels * Ypixels * k] += Mass[n]*Quantity2[n]*wk / sum;
                                ValueQuantity3[i + Xpixels * j + Xpixels * Ypixels * k] += Mass[n]*Quantity3[n]*wk / sum;
                                ValueQuantity4[i + Xpixels * j + Xpixels * Ypixels * k] += Mass[n]*Quantity4[n]*wk / sum;
                              }
                          }
                  }
              }
  ...
  for(i = 0; i < Xpixels; i++)
    for(j = 0; j < Ypixels; j++)
	  ...
	  {
          ValueQuantity1[i + Xpixels * j + Xpixels * Ypixels * k] /= Value[i + Xpixels * j + Xpixels * Ypixels * k];
          ValueQuantity2[i + Xpixels * j + Xpixels * Ypixels * k] /= Value[i + Xpixels * j + Xpixels * Ypixels * k];
          ValueQuantity3[i + Xpixels * j + Xpixels * Ypixels * k] /= Value[i + Xpixels * j + Xpixels * Ypixels * k];
          ValueQuantity4[i + Xpixels * j + Xpixels * Ypixels * k] /= Value[i + Xpixels * j + Xpixels * Ypixels * k];
	  }
}

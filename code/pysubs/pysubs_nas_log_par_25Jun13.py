...
def FindFom(snapmin,snapmax,Z,phi,theta,psi,RelVel,ConstrainPhi=True,TFudge = 1.0,Mask=(1,0,0,0,0,0,0),MaxShift=0.0,SpectralIndex=3.2):
	...
	# This version first scans through a Theta,Psi matrix to deterine the optimum.
        ...
        (dataA,sigmaA,maskA,maskANull,dataB1,dataB1log,sigmaB1,sigmaB1log,dataB2,sigmaB2,dataB3,sigmaB3,dataC,sigmaC,dataD,sigmaD,maskD,maskDNull,dataE,\
         sigmaE) = GetData()
	...
	while counter < MaxCounter:
		...
		if Vz > MaxVz or Vz < MinVz: # If Vz is outside of observed Mean +/- 4 Sigma
			...
			sumsim.data=gaussian_filter(dmsim.data+masssim.data,2.0)
                        xraysim1.data=log10(xraysim1.data)	
			...
			data2list=list((dataA,dataB1log))
			sigmalist=list((sigmaA,sigmaB1log))
                        if Mask[1] == 1:
			...
			[shifteddata1list,align,fom]=FindBestShift(data1list, data2list, sigmalist, masklist, align, tol)
                        ...
                        xray1chisquared = pow((dataB1log.data-shiftedxsim1.data)/sigmaB1log.data,2) * maskA.data
...
def GetData():
	...
	toppath = lageconfig.toppath
	datapathA=toppath+'bullet/data/paraficz/mass_rescaled.fits'
	datapathB1=toppath+'bullet/data/xray_500_2000_29Jun11.dat'
	datapathB1log=toppath+'bullet/data/xray_500_2000_Log_25Jun13.dat'
	...
	dataA=Array2d(dxminA,dxmaxA,xpixelsA,dyminA,dymaxA,ypixelsA)
        PNormFactor = 100.0
        fits = pyfits.open(datapathA)
        for i in range(dataA.nx):
            for j in range(dataA.ny):
                dataA.data[i,j] = fits[0].data[j,i] * PNormFactor
	...
	dataB1log=Array2d(-dxmaxB,dxmaxB,xpixelsB,-dymaxB,dymaxB,ypixelsB)
	dataB1=GetBulletData(datapathB1,dataB1)# XRay measured data
	dataB1log=GetBulletData(datapathB1log,dataB1log)# XRay measured data
	sigmaB1=Array2d(-dxmaxB,dxmaxB,xpixelsB,-dymaxB,dymaxB,ypixelsB)
        sigmaB1log=Array2d(-dxmaxB,dxmaxB,xpixelsB,-dymaxB,dymaxB,ypixelsB)
	sigmaB1=GetBulletData(datapathsB1,sigmaB1)# XRay sigma
        sigmaB1log.data = dataB1log.data * (-0.01) # Arbitrarily set log sigma
	...
	maskDNull=GetBulletData(datapathmDNull,maskDNull)#  Null Mask
        ...
        return (dataA,sigmaA,maskA,maskANull,dataB1,dataB1log,sigmaB1,sigmaB1log,dataB2,sigmaB2,dataB3,sigmaB3,dataC,sigmaC,dataD,sigmaD,maskD,maskDNull,dataE,sigmaE)
...
def BestPlot(snap,Z,phi=0.0,theta=0.0,psi=0.0,PlotSuffix='New_Kappa',FomLocate=False,TFudge=1.0,ConstrainPhi=False, Mask=(1,0,0,0,0,0),SpectralIndex=3.2,MaxShift=0.0):
	...
	data2list=list((dataA,dataB1log,dataB2,dataB3,dataC,dataD,dataE,dataE))
	sigmalist=list((sigmaA,sigmaB1log,sigmaB2,sigmaB3,sigmaC,sigmaD,sigmaE,sigmaE))
        masklist=list()
	...
	shiftedxsim1=shifteddata1list[1]
        shiftedxsim1.data = pow(10.0,shiftedxsim1.data)
...
def SimpleFom(snap,Z,phi=0.0,theta=0.0,psi=0.0,ConstrainPhi=False,TFudge=1.0):
    ...
    toppath = lageconfig.toppath
    (dataA,sigmaA,maskA,maskANull,dataB1,dataB1log,sigmaB1,sigmaB1log,dataB2,sigmaB2,dataB3,sigmaB3,dataC,sigmaC,dataD,sigmaD,maskD,maskDNull,\
         dataE,sigmaE) = GetData()
    [dyyA,dxxA] = meshgrid(dataA.y,dataA.x)# Data grid for plots
    [dyyB,dxxB] = meshgrid(dataB1.y,dataB1.x)# Data grid for plots
    [dyyD,dxxD] = meshgrid(dataD.y,dataD.x)# Data grid for plots

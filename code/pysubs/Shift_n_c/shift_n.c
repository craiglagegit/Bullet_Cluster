...
{
  int numarrays;
  Arr *data1, *data2, *shifteddata1, *sigma;
};
...

{
  ...
  func.evals=0;
  func.EPS=1.0E-4; // This determines the size of the derivative step
  func.f=0;
  func.align=aptr;
  ...
  for (k=0; k<arrayset->numarrays; k++)
	  ...
	  for (i=0; i<arrayset->data2[k].nx; i++) 
	      ...
	      for (j=0; j<arrayset->data2[k].ny; j++)
		{
		  ...
		  fom[k] = fom[k] + pow((arrayset->shifteddata1[k].data[i+j*arrayset->data2[k].nx] - arrayset->data2[k].data[i+j*arrayset->data2[k].nx]) / arrayset->sigma[k].data[i+j*arrayset->data2[k].nx],2) / (arrayset->data2[k].nx * arrayset->data2[k].ny * arrayset->numarrays);
		}
	    }
	//printf("fom[%d] = %.3f\n",k,fom[k]);
	fomsum = fomsum + fom[k];

...
def ProjectEnzoData(pf,mass,phi=0.0,theta=0.0,psi=0.0,zmin=-3000.0,zmax=3000.0,DMProject=False,center=[0.0,0.0,0.0]):
...
def ProjectEnzoTemp(pf,mass,phi=0.0,theta=0.0,psi=0.0,zmin=-3000.0,zmax=3000.0,center=[0.0,0.0,0.0]):
    # Projects 3D Enzo data onto a 2D grid
...
def FindFom(snapmin,snapmax,Z,phi,theta,psi,RelVel,ConstrainPhi=True,TFudge = 1.0,Mask=(1,0,0,0,0,0,0),MaxShift=0.0,SpectralIndex=3.2):
	...
	[dyyD,dxxD] = meshgrid(dataD.y,dataD.x)# Data grid for plots
        ...
        MinVz = BulletConstants.BulletVz - 8.0 * BulletConstants.BulletSigVz # Minimum Bullet radial velocity consistent with observations
        MaxVz = BulletConstants.BulletVz + 8.0 * BulletConstants.BulletSigVz # Maximum Bullet radial velocity consistent with observations
	...
	PsiMax = int(100*psi) + 9
  	PsiMin = int(100*psi) - 9
	ThetaMax = int(100*theta) + 9
	ThetaMin = int(100*theta) - 9
	...
	for snap in range(snapmin-1,snapmax+2): # First fill the whole array with 1E6 - this IDs the boundaries
		for Psi in range(PsiMin-DeltaPsi, PsiMax+2*DeltaPsi, DeltaPsi):	
			for Theta in range(ThetaMin-DeltaTheta, ThetaMax+2*DeltaTheta, DeltaTheta):
				FomMatrix[snap,Psi,Theta] = (1.0E6,1.0E6,0.0)				
	for snap in range(snapmin,snapmax+1): # Next put -1.0 everywhere except at the boundaries - this IDs conditions not yet run
		for Psi in range(PsiMin, PsiMax+DeltaPsi, DeltaPsi):	
			for Theta in range(ThetaMin, ThetaMax+DeltaTheta, DeltaTheta):
				FomMatrix[snap,Psi,Theta] = (-1.0,-1.0,0.0)		
	snap = snapmin
	...
	while counter < MaxCounter:
		...
		Vz = abs(dot(R,RelVel)[2]) # This is the observed Z_Velocity of the bullet relative to the CM
		if Vz > MaxVz or Vz < MinVz: # If Vz is outside of observed Mean +/- 4 Sigma
			FomMatrix[snap,Psi,Theta] = (1.0E5,1.0E5,0.0) # If it's outside allowed Vz, give it a large FOM
                        fom = 1.0E5
                        Phi = 0.0
			...
			[shifteddata1list,align,fom]=FindBestShift(data1list, data2list, sigmalist, masklist, align, tol)
                        ...
                        print "Xray1 only chisquared = %.4f"%xfom
			FomMatrix[snap,Psi,Theta] = (fom, xfom, align.d[4]) # Store (fom, xfom, phi)
                    except:
			FomMatrix[snap,Psi,Theta] = (1.0E5, 1.0E5, 0.0) # If there's an error, give it a large FOM
                        fom = 1.0E5                              
                        Phi = 0.0
		if fom < bestfom:
			bestfom = fom
				...
				for Th in range(ThetaMin-DeltaTheta, ThetaMax+DeltaTheta, DeltaTheta):
					if FomMatrix[sn,Ps,Th][0] < fom and FomMatrix[sn,Ps,Th][0] > 0:
						fom = FomMatrix[sn,Ps,Th][0]
                                                xfom = FomMatrix[sn,Ps,Th][1]
						snap = sn
                                                Phi = FomMatrix[sn,Ps,Th][2]
						Theta = Th
						Psi = Ps
...
def BestPlot(snap,Z,phi=0.0,theta=0.0,psi=0.0,PlotSuffix='New_Kappa',FomLocate=False,TFudge=1.0,ConstrainPhi=False, Mask=(1,0,0,0,0,0),SpectralIndex=3.2,MaxShift=0.0):
	...
	try:
            ...
            fig = EdgeDetectionPlot(dataB1,shiftedxsim1,"X-ray Shock Edge Detection",filled_levels=filled_levels,ED_filled_levels=ED_filled_levels,\
...
def MoviePlotsMHD1(snapmin,snapmax,xyarray,zmin,zmax,Z,phi=0.0,theta=0.0,psi=0.0,TFudge=1.0,SpectralIndex=3.6):
	...
	pf = GetPF(snap)
        ...
        RhoFind = pf.h.sphere((0,0,0),(3000.0,"kpc")).quantities["MaxLocation"]("Density")
        RhoValue = RhoFind[0]
        RhoCenter = RhoFind[2:5]
        print "RhoMax is ",RhoValue," at Location ",RhoCenter
        BMaxFind = pf.h.sphere((0,0,0),(3000.0,"kpc")).quantities["MaxLocation"]("BMag")
        BMaxValue = BMaxFind[0]
        BMaxCenter = BMaxFind[2:5]
        print "BMax is ",BMaxValue," at Location ",BMaxCenter
        # For early pictures, put center at [-200,0,0]
	simtime = pf.h.parameters['InitialTime'] * BulletConstants.TimeConversion # Put time in Gyears
	...
	[DMrho,gasrho,xraysim1,xraysim2,xraysim3,szsim]=ProjectEnzoData(pf,gasrho,phi=phi,theta=theta,psi=psi,zmin=zmin,zmax=zmax,center = BMaxCenter)


	[tempsim,bmagsim,synchsim]=ProjectEnzoTemp(pf,gasrho,phi=phi,theta=theta,psi=psi,zmin=zmin/5.0,zmax=zmax/5.0,center = BMaxCenter)
	...
	DMrho.data = gaussian_filter(DMrho.data,1.0) / DensityFactor # Smooth and normalize 
	...
	LogBMax = -4.0
        #BMin = 0.0
        #BMax = 20.0
	print "BMin = %g, BMax = %g"%(bmagsim.data.min(), bmagsim.data.max())
	for i in range(xyarray.nx):
        ...
        ax1 = axes([0.25,0.65,0.5,0.25],aspect=1.0)
	Gaslevels=linspace(LogRhoMin,LogRhoMax,200) 
	for i in range(200): Gaslevels[i]=int(Gaslevels[i]*200)/200.0
	con1 = ax1.contourf(dxx,dyy,log10(gasrho.data),Gaslevels,cmap=cm.spectral)
	for c in con1.collections:
		c.set_linewidth(0.1)
		c.set_alpha(1.0)
	DMRholevels=linspace(-26.0,-23.0,12)  
        ax1.contour(dxx,dyy,log10(DMrho.data),DMRholevels,linestyle='-',colors='w', linewidth=2.5)
	ax1.set_xticks([])
	ax1.set_yticks([-1000.0,0.0,1000.0])
	ax1.set_ylabel('kpc')
        c1 = colorbar(con1, pad = 0.02)

        ax2 = axes([0.25,0.375,0.5,0.25],aspect=1.0)
	Templevels=linspace(LogTMin,LogTMax,200) 
	for i in range(200): Templevels[i]=int(Templevels[i]*200)/200.0
        con2 = ax2.contourf(dxx,dyy,log10(tempsim.data),Templevels,cmap=cm.spectral)
	for c in con2.collections:
    		c.set_linewidth(0.1)
    		c.set_alpha(1.0)
	ax2.set_xticks([])
	ax2.set_yticks([-1000.0,0.0,1000.0])
	ax2.set_ylabel('kpc')
        c2 = colorbar(con2, pad = 0.02)

        ax3 = axes([0.25,0.10,0.5,0.25],aspect=1.0)
	Blevels=linspace(LogBMin,LogBMax,200) 
	for i in range(200): Blevels[i]=int(Blevels[i]*200)/200.0
        con3 = ax3.contourf(dxx,dyy,log10(bmagsim.data),Blevels,cmap=cm.spectral)
	#Blevels=linspace(BMin,BMax,200) 
	#for i in range(200): Blevels[i]=int(Blevels[i]*200)/200.0
        #con3 = ax3.contourf(dxx,dyy,bmagsim.data*1.0E6,Blevels,cmap=cm.spectral)
	for c in con3.collections:
    		c.set_linewidth(0.1)
    		c.set_alpha(1.0)
	ax3.set_xticks([-1000.0,0.0,1000.0])
	ax3.set_xlabel('kpc')
	ax3.set_yticks([-1000.0,0.0,1000.0])
	ax3.set_ylabel('kpc')
        c3 = colorbar(con3, pad = 0.02)

        filename = 'movie/collision_center'+str('%03d'%snap)+'.png'

...
import matplotlib
matplotlib.use("PDF")
import pysubs_nas_19Nov12 as pysubs
from pylab import *
...
if rank == 0:
	comm.send([bestmassx1fom,bestmassx1snap,bestmassx1time,bestmassx1theta,bestmassx1psi], dest=1, tag=2000)
	Mask=(1,0,0,0,0,0)
	(bestmassfom,bestmasssnap,bestmassphi,bestmasstheta,bestmasspsi,bestmasstime,counter) = pysubs.FindFom(max(snapmin,bestmasssnap-1),min(snapmax,bestmasssnap+2),Z,phi,bestmasstheta,bestmasspsi,ConstrainPhi=ConstrainPhi, Mask=Mask)
	...
	fomfile.write(result)
	fomfile.close
	AllDone = comm.recv(source=1, tag=2001)
	...
	[bestmassx1fom,bestmassx1snap,bestmassx1time,bestmassx1theta,bestmassx1psi] = comm.recv(source=0, tag=2000)
	Mask=(1,1,0,0,0,0)
	(bestmassx1fom,bestmassx1snap,bestmassx1phi,bestmassx1theta,bestmassx1psi,bestmassx1time,counter) = pysubs.FindFom(max(snapmin,bestmassx1snap-1),min(snapmax,bestmassx1snap+2),Z,phi,bestmassx1theta,bestmassx1psi,ConstrainPhi=ConstrainPhi, Mask=Mask)
	...
	fomfile.write(result)
	fomfile.close
	comm.send(True, dest=0, tag=2001)
	print "Processor %d - Normal Exit\n"%rank

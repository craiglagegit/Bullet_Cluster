...
from cysubs_nas_17Oct12 import FindEnzoCentroids
from pylab import *
...
for snap in range(snapmin, snapmax):
	if rank == 0:
		...
		try:
			...
			[NumPart, Masses, Centroids] = FindEnzoCentroids(pf)
...
if rank == 0:
	...
	fomfile=open('newfom_mass_redo.out','w')
	fomfile.write(result)
	fomfile.close
	...
	Mask=(1,1,0,0,0,0)
	(bestmassx1fom,bestmassx1snap,bestmassx1psi,bestmassx1theta,bestmassx1time,counter) = pysubs.FindFom(max(snapmin,bestmassx1snap-1),min(snapmax,bestmassx1snap+2),Z,phi,bestmassx1theta,bestmassx1psi,ConstrainPhi=ConstrainPhi, Mask=Mask)
	result="FOM=%.6f, BestSnap = %d, BestTime =  %.4f, BestPsi = %.3f, BestTheta = %.3f\n"%(bestmassx1fom,bestmassx1snap,bestmassx1time,bestmassx1psi,bestmassx1theta)
	PlotSuffix = 'MassX1_Redo'
	...
	fomfile=open('newfom_massx1_redo.out','w')
	fomfile.write(result)
	fomfile.close
	print "Processor %d - Normal Exit\n"%rank
else:

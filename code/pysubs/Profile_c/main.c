...
void Profile(int numpart, struct particle_data *p, float *hsml, float *mass, float *quantity, float xcen, float ycen, float zcen, float rmax, int rpixels, int desdensngb, float hmax, double boxsize, float *value, float *valuequantity)
{
...
void make_map(void)
{
  ...
  for(n = 0; n < NumPart; n++)
      ...
      if((n % (NumPart / 100)) == 0)
	{
      ...
      for(dR = -nR; dR <= nR; dR++)
            {
              r = fabs(Rcen + dR * pixelsizeR - Rpart);

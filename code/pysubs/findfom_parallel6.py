...
for snap in range(snapmin, snapmax):
	...
	if not StartedAngleRuns: # This loop only gets run the first time through
		angles = list()
		if rank == 0:
			MinInitVz = BulletConstants.BulletVz - 8.0 * BulletConstants.BulletSigVz # Minimum Bullet radial velocity plus margin
			MaxInitVz = BulletConstants.BulletVz + 8.0 * BulletConstants.BulletSigVz # Maximum Bullet radial velocity plus margin
			...
			for Theta in range(int(100*thetamin), int(100*thetamax), 20):
				...
				for Psi in range(int(100*psimin), int(100*psimax), 20):
					...
					if Vz < MaxInitVz and Vz > MinInitVz: # If Vz is inside of observed Mean +/- 8 Sigma
...
if rank == 0:
	...
	Mask=(1,1,0,0,0,0)
	(bestmassx1fom,bestmassx1snap,bestmassx1phi,bestmassx1theta,bestmassx1psi,bestmassx1time,counter) = pysubs.FindFom(max(snapmin,bestmasssnap-1),min(snapmax,bestmasssnap+2),Z,phi,bestmasstheta,bestmasspsi,bestmassrelvel,ConstrainPhi=ConstrainPhi, Mask=Mask)
	result="FOM=%.6f, BestSnap = %d, BestTime =  %.4f, BestPhi = %.3f, BestTheta = %.3f, BestPsi = %.3f\n"%(bestmassx1fom,bestmassx1snap,bestmassx1time,bestmassx1phi,bestmassx1theta,bestmassx1psi)
	PlotSuffix = 'MassX1P'
	pysubs.BestPlot(bestmassx1snap,Z,phi,bestmassx1theta,bestmassx1psi,PlotSuffix=PlotSuffix,FomLocate=True,ConstrainPhi=ConstrainPhi,Mask=Mask)

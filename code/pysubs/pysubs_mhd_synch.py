...
def FindGadgetCentroids(snapfile): 
	...
	while (Masses[parttype,1] == 0):
		pc = offset + int(Npart[parttype] * rand())
...
def ProjectEnzoData(pf,mass,phi=0.0,theta=0.0,psi=0.0):
    ...
    add_field('Synch', function = _EnzoSynch, units=r"\rm{cm}^{-3}\rm{s}^{-1}", projected_units=r"\rm{cm}^{-2}\rm{s}^{-1}")
    ...
    pf.field_info['Synch'].take_log=False
    center = [(mass.xmin+mass.xmax)/2.0,(mass.ymin+mass.ymax)/2.0,(zmin+zmax)/2.0] # Data Center
    normal_vector=(0.0,0.0,1.0)
    north_vector = (0.0,-1.0,0.0)
    R = EulerAngles(phi,theta,psi)
    normal_vector = center + dot(R,normal_vector)
    north_vector = center + dot(R,north_vector)
    ...
    vp1 = pf.h.volume_rendering(normal_vector, width, center, resolution, tf, fields=['Density','SZ','Synch'], north_vector=north_vector)
    vp1.ray_cast()
    ...
    Synch.data = vp1.image[:,:,2] * SynchFactor 

    elapsed = (time.time()-start)
    print "Elapsed time to run Enzo gas projection = "+str(elapsed)

    return [mass,Xray1,Xray2,Xray3,SZ,Synch]

def ProjectEnzoTemp(pf,mass,phi=0.0,theta=0.0,psi=0.0):
    # Projects 3D Enzo data onto a 2D grid
    # Returns mass data, Three Xray intensities, and SZ data.
    ...
    vp1 = pf.h.volume_rendering(normal_vector, width, center, resolution, tf, fields=['TXRay','XRay'], north_vector=north_vector)
    vp1.ray_cast()
...
def ProjectEnzoBMag(pf,mass,phi=0.0,theta=0.0,psi=0.0):
    # Projects 3D Enzo data onto a 2D grid
    # Returns mass data, Three Xray intensities, and SZ data.
    ...
    vp1 = pf.h.volume_rendering(normal_vector, width, center, resolution, tf, fields=['BMag'], north_vector=north_vector)
    vp1.ray_cast()


    BMag.data = vp1.image[:,:,0]

    elapsed = (time.time()-start)
    print "Elapsed time to run Enzo projection = "+str(elapsed)
...
def _EnzoSynch(field,data): 
	P = 2.85 # P is the power law exponent
...
def _EnzoBMag(field,data): 
	BMag = sqrt(data['Bx']**2 + data['By']**2 + data['Bz']**2) 
	return  BMag	
...
def ProjectEnzoDM(pf,data,parttype,zmin,zmax,phi=0.0,theta=0.0,psi=0.0): 
    ...
    for i in range(xpixels):
	for j in range(ypixels):
            data.data[i,j] = valueptr[j+i*ypixels]#valueptr[i+j*xpixels]  - Why did I need to flip these?
...
def FindFom(toppath,dir,snapmin,snapmax,counter,phi=0.0,theta=0.0,psi=0.0,PlotSuffix='Test',simulator="Gadget",PDF=True, FomLocate=False):
	...
	datapathE=toppath+'bullet/data/radio_5Oct11.dat'
	...
	maskE=Array2d(-dxmaxA,dxmaxA,xpixelsA,-dymaxA,dymaxA,ypixelsA)
	...
	for snap in range(snapmin,snapmax): # Which snapshot files
		...
		if simulator == "Gadget":
			...
			[sumsim,xraysim1,xraysim2,xraysim3,szsim,synchsim]=ProjectEnzoData(pf,masssim,phi,theta,psi)
			[tempsim]=ProjectEnzoTemp(pf,tempmass,phi,theta,psi)
			[bmagsim]=ProjectEnzoBMag(pf,masssim,phi,theta,psi)
			print tempsim.data.min(), tempsim.data.max(), synchsim.data.min(), synchsim.data.max()
			print bmagsim.data.shape, bmagsim.data.min(), bmagsim.data.max()
			sumsim.data=gaussian_filter(sumsim.data,2.0)
			print "Total Mass = %.3g" %sumsim.data.sum()
			# Projected Enzo sim data
			
			if cos(phi)>=0.0:
				align.d[2] = 0.22  - pi/2 - arctan( cos(theta)*tan(phi) ) # Seed psi close to final result
			else:
				align.d[2] = 0.22  + pi/2 - arctan( cos(theta)*tan(phi) ) # Seed psi close to final result
		...
		masklist=list((maskA,maskA,maskA,maskA,maskA,maskD,maskE,maskE))
		...
		if PlotSuffix != None:
			try:
			    ...
			    subplot("224")
			    title('Axial Profile ($10^{10} M_\odot/pixel$)')
			    ...
			    subplot("221")
			    title("Log X-ray data-500-2000eV")
			    XrayLogMin = -10.0
			    XrayLogMax = -4.0
			    ...
			    subplot("222")
			    title("Log X-ray simulation-500-2000eV")
			    ...
			    subplot("224")
			    title('Axial Profile ($10^{-6}photons/cm^2 sec$)')
			    plot(dataslice.x*1E6,dataslice.data*1E6,label="Data",linewidth=3.0)
			    plot(simslice.x*1E6,simslice.data*1E6,label="Sim",linewidth=3.0)
			    ...
			    subplot("221")
			    title("Log X-ray data-2000-5000eV")
			    XrayLogMin = -10.0
			    XrayLogMax = -4.0
			    Xraylevels=linspace(XrayLogMin,XrayLogMax,25) 
			    for i in range(25): Xraylevels[i]=int(Xraylevels[i]*10)/10.0
			    ...
			    subplot("222")
			    title("Log X-ray simulation-2000-5000eV")
			    ...
			    subplot("221")
			    title("Log X-ray data-5000-8000eV")
			    XrayLogMin = -10.0
			    XrayLogMax = -4.0
			    Xraylevels=linspace(XrayLogMin,XrayLogMax,25) 
			    for i in range(25): Xraylevels[i]=int(Xraylevels[i]*10)/10.0
			    ...
			    subplot("222")
			    title("Log X-ray simulation-5000-8000eV")
			    ...
			    subplot("224")
			    title('Axial Profile ($10^{-7}photons/cm^2 sec$)')
			    plot(dataslice.x*1E7,dataslice.data*1E7,label="Data",linewidth=3.0)
			    plot(simslice.x*1E7,simslice.data*1E7,label="Sim",linewidth=3.0)
			    ...
			    [dataslice,simslice] = OneDSlice2Fixed(dataE,shiftedsynchsim,dataslice,simslice,0.24,50,52)
			    ...
			    Blevels = [0.0,5.0,10.0,15.0,20.0]
			    contourf(dxxA,dyyA,dataE.data,radiolevels,cmap=cm.spectral)
			    for c in contourf(dxxA,dyyA,dataE.data,radiolevels,cmap=cm.spectral).collections:
				c.set_linewidth(0.1)
				c.set_alpha(1.0)
...
def MoviePlots5(snapdir,dir,snapmin,snapmax,counter,xyarray,zmin,zmax,PDF=False):
	...
	title("Log X-ray Intensity", fontsize=12)
	Xraylevels=linspace(XrayLogMin,XrayLogMax,200) 
...
def MoviePlotsMHD1(snapdir,dir,snapmin,snapmax,counter,xyarray,zmin,zmax,PDF=False):
	...
	xraysim=Array2d(xyarray.xmin,xyarray.xmax,xyarray.nx,xyarray.ymin,xyarray.ymax,xyarray.ny)
        ...
        try:
	    [sumsim,xraysim1,xraysim2,xraysim3,Temp,szsim]=ProjectEnzoData(filename,masssim)
        except:
        ...
        simtime=snap*.01*BulletConstants.TimeConversion # Put time in Gyears
        PixelVolume=xyarray.dx * xyarray.dy 
	sumsim.data=gaussian_filter(sumsim.data,0.5) / PixelVolume # Smooth and normalize

	xraysim.data=gaussian_filter(xraysim1.data+xraysim2.data+xraysim3.data,0.5)
	...
	for i in range(xyarray.nx):
		for j in range(xyarray.ny):
			xraysim.data[i,j]=max(xraysim.data[i,j],1.01*10**XrayLogMin)
			sumsim.data[i,j]=max(sumsim.data[i,j],1.01*10**LogRhoMin)
			Temp.data[i,j]=max(Temp.data[i,j],1.01*10**LogTMin)
	...
	Templevels=linspace(LogTMin,LogTMax,100) 
	for i in range(100): Templevels[i]=int(Templevels[i]*100)/100.0
        contourf(dxx,dyy,log(Temp.data)/log(10),Templevels,cmap=cm.spectral)
	for c in contourf(dxx,dyy,log(Temp.data)/log(10),Templevels,cmap=cm.spectral).collections:
    		c.set_linewidth(0.1)
    		c.set_alpha(1.0)
        colorbar()
	gca().set_xticks([-1000.0,0.0,1000.0])
	...
	if PDF:
	        ...
	        filename = dir+'movie2/collision'+str('%03d'%snap)+'.png'
		plt.savefig(filename)#, size=(2400,1800))
	        print 'Wrote file', filename
	        plt.clf()

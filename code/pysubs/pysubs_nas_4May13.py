...
def FindFom(snapmin,snapmax,Z,phi,theta,psi,RelVel,ConstrainPhi=True,TFudge = 1.0,Mask=(1,0,0,0,0,0,0),MaxShift=0.0,SpectralIndex=3.2):
	...
	PsiMax = int(100*psi) + 60
  	PsiMin = int(100*psi) - 60
	ThetaMax = int(100*theta) + 60
	ThetaMin = int(100*theta) - 60
	...
	while counter < MaxCounter:
		...
		if Vz > MaxVz or Vz < MinVz: # If Vz is outside of observed Mean +/- 4 Sigma
			...
			FomMatrix[snap,Psi,Theta] = (fom, align.d[4]) # Store (fom, phi)
                    except:
...
def BestPlot(snap,Z,phi=0.0,theta=0.0,psi=0.0,PlotSuffix='New_Kappa',FomLocate=False,TFudge=1.0,ConstrainPhi=False, Mask=(1,0,0,0,0,0),SpectralIndex=3.2,MaxShift=0.0):
        ...
        (dataA,sigmaA,maskA,maskANull,dataB1,sigmaB1,dataB2,sigmaB2,dataB3,sigmaB3,dataC,sigmaC,dataD,sigmaD,maskD,maskDNull,dataE,sigmaE) = GetData()
        (szekernel, kernel60, kernel15, kernel20) = GetKernels()
	[dyyA,dxxA] = meshgrid(dataA.y,dataA.x)# Data grid for plots
	[dyyB,dxxB] = meshgrid(dataB1.y,dataB1.x)# Data grid for plots
	[dyyD,dxxD] = meshgrid(dataD.y,dataD.x)# Data grid for plots
	...
	try:
	    ...
	    figure()
	    suptitle('Log X-ray Flux 500-2000eV, T = %.2f Gy'%simtime, fontsize=16)
	    subplots_adjust(hspace=0.2, wspace=0.0)
	    ...
	    subplot("224")
	    title('1D Slice (linear)')
	    plot(dataslice.x,dataslice.data*1.0E6,label="Data",linewidth=3.0)
	    plot(simslice.x,simslice.data*1.0E6,label="Sim",linewidth=3.0)

	    xlabel('kpc')
	    legend(loc=1,bbox_to_anchor=[0.45, 1.0])
	    ltext = gca().get_legend().get_texts()
	    setp(ltext, fontsize = 9, color = 'k')
	    pp.savefig()
	    ...
	    figure()
	    suptitle('Log X-ray Flux 2000-5000eV, T = %.2f Gy'%simtime, fontsize=16)
	    subplots_adjust(hspace=0.2, wspace=0.00)
	    ...
	    figure()
	    suptitle('Log X-ray Flux 5000-8000eV, T = %.2f Gy'%simtime, fontsize=16)
	    subplots_adjust(hspace=0.2, wspace=0.0)
	    ...
	    subplot("224")
	    title('1D Slice (linear)')
	    plot(dataslice.x,dataslice.data*1.0E7,label="Data",linewidth=3.0)
	    plot(simslice.x,simslice.data*1.0E7,label="Sim",linewidth=3.0)
	    ...
	    figure()
	    suptitle('X-ray Shock Location, T = %.2f Gy'%simtime, fontsize=16)
	    subplots_adjust(hspace=0.2, wspace=0.0)
		...
		c.set_alpha(1.0)
	    contour(dxxB,dyyB,log10(shiftedxsim1.data),XraylevelsCon,linestyle='-',colors='k')
	    plot(dataslice1.x,dataslice1.y,'w--',linewidth=2.0)# Plot the 1D slice
	    plot(dataslice2.x,dataslice2.y,'y--',linewidth=2.0)# Plot the 1D slice
	    plot(dataslice3.x,dataslice3.y,'k--',linewidth=2.0)# Plot the 1D slice
	    gca().set_xticks([])
	    ylabel('kpc')

	    subplot("222")
	    title('1D Slice (Log - White)')
	    ...
	    gca().set_xticks([])
	    gca().set_yticks([])
	    legend(loc=1,bbox_to_anchor=[0.60, 0.32])
	    ltext = gca().get_legend().get_texts()
	    setp(ltext, fontsize = 9, color = 'k')

	    subplot("223")
	    title('1D Slice (Log - Yellow)')
	    ...
	    gca().set_xticks([0.0,500.0,1000.0])
	    xlabel('kpc')
	    gca().set_yticks([])
	    legend(loc=1,bbox_to_anchor=[0.60, 0.32])
	    ltext = gca().get_legend().get_texts()
	    ...
	    figure()
	    suptitle('X-ray Shock Edge Detection, T = %.2f Gy'%simtime, fontsize=16)
	    subplots_adjust(hspace=0.2, wspace=0.0)
	    subplot("221",aspect=1)
	    title("Data")
	    XrayLogMin = -10.0
	    ...
	    Xraylevels=linspace(XrayLogMin,XrayLogMax,25) 
	    for i in range(25): Xraylevels[i]=int(Xraylevels[i]*10)/10.0
	    EDlevels=linspace(0,10.0,11) 
            ContourStep = 1.0E-7
	    XraylevelsCon=[1.0*ContourStep,2.0*ContourStep,3.0*ContourStep,4.0*ContourStep,5.0*ContourStep]
	    contourf(dxxB,dyyB,log10(dataB1.data),Xraylevels,cmap=cm.spectral)
	    for c in contourf(dxxB,dyyB,log10(dataB1.data),Xraylevels,cmap=cm.spectral).collections:
		c.set_linewidth(0.1)
		c.set_alpha(1.0)
	    ...
	    simedge=Array2d(dataB1.xmin,dataB1.xmax,dataB1.nx,dataB1.ymin,dataB1.ymax,dataB1.ny)
	    simtrunc=Array2d(dataB1.xmin,dataB1.xmax,dataB1.nx,dataB1.ymin,dataB1.ymax,dataB1.ny)
	    ...
	    subplot("222",aspect=1)
	    title("Data")
	    contourf(dxxB,dyyB,dataedge.data*1.0E7,EDlevels,cmap=cm.spectral)
	    for c in contourf(dxxB,dyyB,dataedge.data*1.0E7,EDlevels,cmap=cm.spectral).collections:
		c.set_linewidth(0.1)
		c.set_alpha(1.0)
	    colorbar()
	    gca().set_xticks([])
	    ...
	    subplot("223",aspect=1)
	    title("Simulation")
	    contourf(dxxB,dyyB,simedge.data*1.0E7,EDlevels,cmap=cm.spectral)
	    for c in contourf(dxxB,dyyB,simedge.data*1.0E7,EDlevels,cmap=cm.spectral).collections:
		c.set_linewidth(0.1)
		c.set_alpha(1.0)
	    gca().set_xticks([-800.0,-400.0,0.0,800.0,400.0])
	    ylabel('kpc')
		...
		c.set_linewidth(0.1)
		c.set_alpha(1.0)
	    contour(dxxB,dyyB,simedge.data,XraylevelsCon,linestyle='-',colors='k')
	    gca().set_xticks([-800.0,-400.0,0.0,800.0,400.0])
	    gca().set_yticks([])
	    xlabel('kpc')
	    pp.savefig()
	    ...
	    figure()
	    suptitle('Gas Temperature (keV), T = %.2f Gy'%simtime, fontsize=16)
	    subplots_adjust(hspace=0.2, wspace=0.00)
	    ...
	    gca().set_xticks([-800.0,-400.0,0.0,800.0,400.0])
	    xlabel('kpc')
	    legend(loc=1,bbox_to_anchor=[0.6, 1.0])
	    ltext = gca().get_legend().get_texts()
	    setp(ltext, fontsize = 9, color = 'k')	    
	    pp.savefig()
	    ...
	    # Now the SZE plots
	    figure()
	    suptitle('SZE Temperature Decrement($\mu K$), T = %.2f Gy'%simtime, fontsize=16)
	    subplots_adjust(hspace=0.2, wspace=0.0)
	    ...
	    gca().set_xticks([])
	    gca().set_yticks([])
	    [dataslice,simslice] = OneDSlice2Fixed(dataC,shiftedszsim,0.24,70,59)
	    ...
	    xlabel('kpc')
	    legend(loc=1,bbox_to_anchor=[0.7, 0.32])
	    ltext = gca().get_legend().get_texts()
	    setp(ltext, fontsize = 9, color = 'k')

	    pp.savefig()
	    
            # Now the Synchrotron plots                                                                                                      
            figure()
            #radiolevels = [0.0,270.0, 540.0, 1080.0, 1620.0, 2160.0, 4000.0]
            radiolevels = [0.0,6.0, 12.0, 24.0, 36.0, 48.0, 96.0]
            suptitle('Radio Flux - 1.3 GHz ($\mu Jy/pixel$) , T = %.2f Gy'%simtime, fontsize=16)
	    subplots_adjust(hspace=0.2, wspace=0.0)
            subplot("221",aspect=1)
            title("Data ")
            contourf(dxxA,dyyA,dataE.data,radiolevels,cmap=cm.spectral)
            for c in contourf(dxxA,dyyA,dataE.data,radiolevels,cmap=cm.spectral).collections:
                c.set_linewidth(0.1)
                c.set_alpha(1.0)
            gca().set_xticks([])
            ylabel('kpc')
            ...
            subplot("222",aspect=1)
            title("Simulation")
            contourf(dxxA,dyyA,shiftedsynchsim.data,cmap=cm.spectral)
            for c in contourf(dxxA,dyyA,shiftedsynchsim.data,radiolevels,cmap=cm.spectral).collections:
                c.set_linewidth(0.1)
                c.set_alpha(1.0)
            colorbar()
            gca().set_xticks([])
	    gca().set_yticks([])
            ...
            [dataslice,simslice] = OneDSlice2Fixed(dataE,shiftedsynchsim,0.24,70,59)
            subplot("223",aspect=1)
            title('Overlay')
            ...
            for c in contourf(dxxA,dyyA,dataE.data,radiolevels,cmap=cm.spectral).collections:
                c.set_linewidth(0.1)
                c.set_alpha(1.0)
            contour(dxxA,dyyA,shiftedsynchsim.data,radiolevels,linestyle='-',colors='k')
            plot(dataslice.x,dataslice.y,'w--', linewidth=2.0)# Plot the 1D slice                                                            
            gca().set_xticks([-800.0,-400.0,0.0,800.0,400.0])
            xlabel('kpc')
            ylabel('kpc')
            ...
            subplot("224")
            title('1D Slice ($\mu Jy/pixel$)')
            plot(dataslice.x,dataslice.data,label="Data",linewidth=3.0)
            plot(simslice.x,simslice.data,label="Sim",linewidth=3.0)
            
            xlabel('kpc')
            legend(loc=1,bbox_to_anchor=[1.0, 1.0])
            ltext = gca().get_legend().get_texts()
            setp(ltext, fontsize = 9, color = 'k')
            
            pp.savefig()
            """
            # Now the Synchrotron/BField overlay                                                                                             
            figure()
            radiolevels = [0.0,6.0, 12.0, 24.0, 36.0, 48.0, 96.0]
            suptitle('Bullet Cluster Collision, T = %.2f Gy'%simtime, fontsize=16)
	    subplots_adjust(hspace=0.2, wspace=0.05)
            subplot("221",aspect=1)
            title("Radio data ($\mu Jy/pixel$)")
            contourf(dxxA,dyyA,dataE.data,radiolevels,cmap=cm.spectral)
            for c in contourf(dxxA,dyyA,dataE.data,radiolevels,cmap=cm.spectral).collections:
                c.set_linewidth(0.1)
                c.set_alpha(1.0)
            ...
            subplot("222",aspect=1)
            title("Radio simulation ($\mu Jy/pixel$)")
            contourf(dxxA,dyyA,shiftedsynchsim.data,cmap=cm.spectral)
            for c in contourf(dxxA,dyyA,shiftedsynchsim.data,radiolevels,cmap=cm.spectral).collections:
                c.set_linewidth(0.1)
                c.set_alpha(1.0)
	    ...
	    gca().set_yticks([])
            ...
            subplot("223",aspect=1)
            title('Radio Data with BField Overlay')
            Blevels = [0.0,0.5,1.0,1.5,2.0, 2.5]#[0.0,5.0,10.0,15.0,20.0]
            contourf(dxxA,dyyA,dataE.data,radiolevels,cmap=cm.spectral)
            for c in contourf(dxxA,dyyA,dataE.data,radiolevels,cmap=cm.spectral).collections:
                c.set_linewidth(0.1)
                c.set_alpha(1.0)
            contour(dxxA,dyyA,shiftedbmagsim.data*1E6,Blevels,linestyle='-',colors='w')
            gca().set_xticks([-800.0,-400.0,0.0,800.0,400.0])
            xlabel('kpc')
            
            subplot("224")
            title('Radio Simulation with BField Overlay')
            Blevels = [0.0,0.5,1.0,1.5,2.0, 2.5]#[0.0,5.0,10.0,15.0,20.0]
            contourf(dxxA,dyyA,shiftedsynchsim.data,radiolevels,cmap=cm.spectral)
            for c in contourf(dxxA,dyyA,shiftedsynchsim.data,radiolevels,cmap=cm.spectral).collections:
                c.set_linewidth(0.1)
                c.set_alpha(1.0)
            contour(dxxA,dyyA,shiftedbmagsim.data*1E6,Blevels,linestyle='-',colors='w')
	    ...
	    if FomLocate:
		    ...
		    figure()
		    suptitle('$\chi^2$ Contribution by Location, T = %.2f Gy'%simtime, fontsize=16)
                    subplots_adjust(hspace=0.2 , wspace=0.00)

		    subplot(2,3,1,aspect=1)
		    title("Mass $\chi^2$=%.2f"%masscontrib)
		    ...
		    title("X-ray 1 $\chi^2$=%.2f"%xray1contrib)
		    ...
		    title("X-ray 2 $\chi^2$=%.2f"%xray2contrib)
		    ...
		    title("X-ray 3 $\chi^2$=%.2f"%xray3contrib)
		    ...
		    xlabel('kpc')
                    ylabel('kpc')
		    gca().set_yticks([-500.0,0.0,500.0])

		    subplot(2,3,5,aspect=1)
		    title("Temp $\chi^2$=%.2f"%tempcontrib)
		    ...
		    subplot(2,3,6,aspect=1)
		    title("SZE $\chi^2$=%.2f"%szecontrib)
...
def SimpleFom(snap,Z,phi=0.0,theta=0.0,psi=0.0,ConstrainPhi=False,TFudge=1.0):
    ...
    try:
            ...
            [shifteddata1list,align,fommassx1]=FindBestShift(data1list, data2list, sigmalist, masklist, align, tol)

            return fommassx1
    except:
        print "Error in SimpleFom routine",sys.exc_info()[0]
        return 1.0E5 # If there's an error, give it a large FOM
...
def MoviePlotsMHD1(snapmin,snapmax,xyarray,zmin,zmax,Z,phi=0.0,theta=0.0,psi=0.0,TFudge=1.0):
	...
	[tempsim,bmagsim,synchsim]=ProjectEnzoTemp(pf,gasrho,phi,theta,psi)

...
extern "C" int optimize(Arr*, Arr*, Arr*, Align*, double*, const double);

//THIS IS THE TOP LEVEL METHOD

extern "C" int optimize(Arr* d1ptr, Arr* sd1ptr, Arr* d2ptr, Align* aptr, double *fptr, const double gtol)
// This is a wrapper function to translate the Python call
// to use the appropriate c++ optimization routines.

{
  Func func(d1ptr, sd1ptr, d2ptr, aptr);
...
Func::Func(Arr* d1ptr, Arr* sd1ptr, Arr* d2ptr, Align* aptr)
// Func constructor
{
  evals=0;
  EPS=1.0E-4;
...

{
  ...
  for (i=0; i<data2->nx; i++) 
    {
      x=data2->x[i];
      for (j=0; j<data2->ny; j++)
	{
	  y=data2->y[j];
	  xprime=x*cos(align->d[2])+y*sin(align->d[2])+align->d[0];
	  yprime=-x*sin(align->d[2])+y*cos(align->d[2])+align->d[1];
	  shifteddata1->data[i+j*data2->nx]=interp(data1,xprime,yprime);
	  fom = fom + pow((shifteddata1->data[i+j*data2->nx]-data2->data[i+j*data2->nx]),2);
	}
    }
  return fom;
}

...
int grid::AddViscosity()
{
  ...
  this->ComputeViscosity(viscosity, DensNum, TENum, GENum, Vel1Num, Vel2Num, Vel3Num, B1Num, B2Num, B3Num); 
...
int grid::ComputeViscosity(float *viscosity, int DensNum, int TENum, int GENum, int Vel1Num, int Vel2Num, int Vel3Num, int B1Num, int B2Num, int B3Num)
{
  ...
  else if (UseViscosity == 3)   {
    // Here we calculate the Spitzer Viscosity, which is nu = const * T^(5/2) / rho
    // Here ViscosityCoefficient is defined as a multiple of Spitzer - typically it will be less than 1
    // The MaxVisc number is arbitrary at this point
    double Spitzer = 6.0E-17 * pow(TemperatureUnits, 5.0 / 2.0) * TimeUnits / (pow(LengthUnits, 2) * DensityUnits);
    double MaxVisc = 1.0E6;
    double emin = SmallT / (Mu * (Gamma - 1.0));
    float rho, etot, eint, temp, v2, b2;
    for (int k = GridStartIndex[2]; k <= GridEndIndex[2]; k++) {
      for (int j = GridStartIndex[1]; j <= GridEndIndex[1]; j++) {
	for (int i = GridStartIndex[0]; i <= GridEndIndex[0]; i++, n++) {
	  igrid = i + (j + k * GridDimension[1]) * GridDimension[0];
	  rho = max(BaryonField[DensNum][igrid], SmallRho);
	  if (DualEnergyFormalism) {
	    eint = max(BaryonField[GENum][igrid], emin);
	  }
	  else {
	    v2 = pow(BaryonField[Vel1Num][igrid],2) + pow(BaryonField[Vel2Num][igrid],2) +pow(BaryonField[Vel3Num][igrid],2);
	    b2 = pow(BaryonField[B1Num][igrid],2) + pow(BaryonField[B2Num][igrid],2) +pow(BaryonField[B3Num][igrid],2);
	    etot = BaryonField[TENum][igrid];
	    eint = max(etot - 0.5*v2 - 0.5*b2/rho, emin); 
	  }
	  temp = Mu * eint * (Gamma - 1.0);
	  viscosity[n] = min(Spitzer * ViscosityCoefficient * pow(temp,5.0 / 2.0) / rho, MaxVisc); 
	  /*
	  if (n % 100000 == 0) {
	   printf("eint: %"GSYM"\n", eint);
	   printf("Temp: %"GSYM"\n", temp);

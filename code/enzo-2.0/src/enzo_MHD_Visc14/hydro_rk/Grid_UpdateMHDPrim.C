...
int grid::UpdateMHDPrim(float **dU, float c1, float c2)
{
  ...
  for (k = GridStartIndex[2]; k <= GridEndIndex[2]; k++) {
	...
	if (D_new < SmallRho || D_new > SmallRho * 1.0E5 || isnan(D_new)) {

	  printf("UpdateMHDPrim: rho <SmallRho at %"ISYM" %"ISYM" %"ISYM": rho_old=%"GSYM", rho=%"GSYM", rho_new=%"GSYM", SmallRho=%"GSYM", dU[iD]=%"FSYM"\n", i, j, k, rho_old, rho, D_new, SmallRho, dU[iD][n]);
	  BaryonField[DensNum][igrid] = 3.0 * SmallRho;
	  BaryonField[Vel1Num][igrid] = 0.0;
	  BaryonField[Vel2Num][igrid] = 0.0;
	  BaryonField[Vel3Num][igrid] = 0.0;
	  BaryonField[TENum][igrid] = emin;
	  BaryonField[GENum][igrid] = emin;
	  BaryonField[B1Num][igrid] = 0.0;
	  BaryonField[B2Num][igrid] = 0.0;
	  BaryonField[B3Num][igrid] = 0.0;
	  BaryonField[PhiNum][igrid] = 0.0;
	  return SUCCESS;
	}
	...
	float etotmin = emin + 0.5*v2 + 0.5*B2/D_new;
	etot = max(etotmin, Tau_new/D_new);
	
	if ((etot < 0 && EOSType == 0) || isnan(etot) || etot > emin * 1.0E5) {
	...
	if (DualEnergyFormalism) {
	  ...
	  if (eint < 0.0 || isnan(eint) || eint > emin * 1.0E5)
	    {

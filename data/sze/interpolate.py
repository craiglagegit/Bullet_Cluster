...
for i in range(oldpixels):
	for j in range(oldpixels):
		offset = 220
		OriginalData.data[i,j]=kernel[i+offset,j+offset]
...
for i in range(3,newpixels-3):
	for j in range(3,newpixels-3):
		NewData.data[i,j]=pysubs.DataInterpolate(OriginalData,NewData.x[i],NewData.y[j]) * oldpixels**2 / newpixels**2

print oldpixels, newpixels, OriginalData.data.sum(), NewData.data.sum()

hdu=pyfits.PrimaryHDU(NewData.data)
hdulist=pyfits.HDUList([hdu])
hdulist.writeto('bullet_transfer_rescaled_small.fits')
